...
// DefaultMaxExpandTargets caps how many hosts a single ExpandTargets
// invocation will return. The bound exists for two reasons: (1) each
// per-host probe runs the full TLS / port-matrix suite which is
// expensive, so a /16 sweep would take hours and burn a lot of
// network; (2) operators paste prefixes by mistake (10.0.0.0/8) more
// often than they mean to, and "the scan errored out immediately
// with a clear message" beats "the scanner ran for two days".
//
// Operators who genuinely want a larger sweep can split the prefix
// into chunks and run them sequentially.
const DefaultMaxExpandTargets = 1024

// ExpandTargets parses a probe target string into a concrete list of
// hosts to scan. Accepted forms (any combination, separated by
// commas / whitespace / semicolons / newlines):
//
//   - CIDR:        "192.168.1.0/24"  → 254 IPs (network + broadcast excluded)
//   - Single IP:   "203.0.113.5"     → 1 host
//   - Hostname:    "example.com"     → 1 host (resolved later by the probe)
//   - Mixed list:  "10.0.0.1, 10.0.0.5/30, host.example.com"
//
// IPv4 CIDRs at /31 and /32 keep all addresses (point-to-point and
// single-host). IPv6 CIDRs are rejected when the prefix is broader
// than /120 — enumerating a /112 is 65k addresses, more than we'll
// run in a single sweep.
//
// Returns ErrTooManyTargets when the expansion would exceed `cap`
// (pass DefaultMaxExpandTargets when no specific bound is needed).
// The returned slice is deduplicated and ordered for deterministic
// scan progression.
func ExpandTargets(raw string, cap int) ([]string, error) {
	if cap <= 0 {
		cap = DefaultMaxExpandTargets
	}
	tokens := splitEndpointCandidates(raw)
	if len(tokens) == 0 {
		return nil, fmt.Errorf("no targets in input")
	}
	...
	for _, tok := range tokens {
		if !strings.Contains(tok, "/") {
			// Bare host or IP — let the existing endpoint parser
			// validate at probe time. We don't gate on host shape
			// here because the probe layer accepts both IPs and DNS
			// names, and downstream parsing is the source of truth.
		...
		expanded, err := expandCIDR(ip, ipNet, cap-len(out))
		if err != nil {
			return nil, err
		}
...
// ErrTooManyTargets is returned by ExpandTargets when the cumulative
// expansion exceeds the caller-supplied cap. The error is detected
// early so the caller can tell the operator without launching any
// probes — large prefixes typically come from typos.
var ErrTooManyTargets = fmt.Errorf("expansion exceeds maximum target count (try a smaller CIDR)")

// expandCIDR turns a parsed CIDR into a slice of host IPs. For IPv4
// blocks larger than /31 it skips the network and broadcast
// addresses (.0 and .255 in /24). For /31 and /32 it keeps every
// address since those are special (point-to-point and single host).
// IPv6 blocks broader than /120 are rejected — enumerating them is
// not practical for an active scan.
func expandCIDR(ip net.IP, ipNet *net.IPNet, remaining int) ([]string, error) {
	ones, bits := ipNet.Mask.Size()
	if bits == 32 {
		// IPv4
		hostBits := bits - ones
		if hostBits > 16 {
			return nil, fmt.Errorf("CIDR %s expands to >65k hosts; %w", ipNet.String(), ErrTooManyTargets)
		}
		size := 1 << hostBits
		// Skip network + broadcast for /30 and wider; keep all for /31 and /32.
		skipEnds := hostBits >= 2
		out := make([]string, 0, size)
		base := ip.Mask(ipNet.Mask).To4()
		if base == nil {
			return nil, fmt.Errorf("CIDR %s is not IPv4", ipNet.String())
		}
		for i := 0; i < size; i++ {
			if skipEnds && (i == 0 || i == size-1) {
				continue
			}
			cur := make(net.IP, 4)
			copy(cur, base)
			carry := uint64(i)
			for byteIdx := 3; byteIdx >= 0 && carry > 0; byteIdx-- {
				sum := uint64(cur[byteIdx]) + (carry & 0xff)
				cur[byteIdx] = byte(sum & 0xff)
				carry = (carry >> 8) + (sum >> 8)
			}
			if len(out)+1 > remaining {
	...
	if bits == 128 {
		// IPv6 — only enumerate /120 or narrower (≤256 addrs).
		hostBits := bits - ones
		if hostBits > 8 {
			return nil, fmt.Errorf("IPv6 CIDR %s broader than /120 not supported; %w",
				ipNet.String(), ErrTooManyTargets)
		}
		size := 1 << hostBits
		out := make([]string, 0, size)
		base := ip.Mask(ipNet.Mask).To16()
		if base == nil {
			return nil, fmt.Errorf("CIDR %s is not IPv6", ipNet.String())
		}
		for i := 0; i < size; i++ {
			cur := make(net.IP, 16)
			copy(cur, base)
			cur[15] = byte(int(cur[15]) + i)
			if len(out)+1 > remaining {
				return nil, ErrTooManyTargets
			}
			out = append(out, cur.String())
	...
	return nil, fmt.Errorf("CIDR %s has unexpected mask width", ipNet.String())
}
