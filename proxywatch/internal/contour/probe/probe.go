	...
	if mode == ProbeModeChecks {
		if err := ctx.Err(); err == nil {
			...
			serviceTargets := []struct {
				...
				// CDN — domain fronting pivot. Targets must be real
				// reachable hosts; bare apex domains like
				// "cloudfront.net" return no server so they always
				// look "blocked".
				{"CFlare", "escape", "www.cloudflare.com", 443},
				{"CFront", "escape", "d1.awsstatic.com", 443}, // CloudFront-fronted AWS static asset host
