...
func ExecuteContext(ctx context.Context, input RunInput) (RunResult, error) {
	...
	if probeMode := NormalizeProbeMode(input.ProbeMode); probeMode != ProbeModeOff {
		...
		summary, probeFindings := probe.RunProbeSuiteWithProgress(ctx, probeMode, input.ProbeRole, input.ProbeTarget, input.Duration, scoped, emit, func(ps ProbeSummary) {
			emitPartial(&ps)
		})
...
func buildFindings(aggs map[string]*candidateAggregate) []Finding {
	...
	for _, agg := range aggs {
		...
		if externalTargets > 0 && internalTargets > 0 {
			sev := "strong"
			if agg.role == "control-pivot" || agg.active {
				sev = "active"
			}
		...
		if externalTargets > 0 && loopTargets >= 2 {
			sev := "strong"
			if agg.active || agg.role == "control-pivot" {
				sev = "active"
			}
		...
		if agg.services != nil && agg.services.Total > 0 {
			...
			if cdnConns > 0 && (agg.writeB >= 50*1024*1024 || agg.role == "control-pivot" || agg.active) {
				sev := "watch"
				if agg.writeB >= 256*1024*1024 || agg.role == "control-pivot" {
					sev = "strong"
				}
