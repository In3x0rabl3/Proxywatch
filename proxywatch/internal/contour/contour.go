...
type Report struct {
	...
	// Probe carries the per-endpoint summary for single-target runs.
	// For CIDR / multi-target sweeps it points at Probes[0] so legacy
	// renderers and saved-report parsers keep working.
	Probe *ProbeSummary `json:"probe,omitempty"`
	// Probes carries one ProbeSummary per scanned host. Populated for
	// all runs (length 1 for single-target, length N for CIDR sweeps).
	// Older saved reports that only persisted Probe still load — we
	// hydrate Probes from Probe when LoadReport sees the legacy shape.
...
// Finding is a type alias for probe.Finding so external callers can
// continue to use contour.Finding without updating their imports.
type Finding = probe.Finding
...
type candidateAggregate struct {
	...
	// Service-aware tracking (populated after probe.ResolveExternalServices).
	services *probe.ServiceProfile
...
func ExecuteContext(ctx context.Context, input RunInput) (RunResult, error) {
	...
	if probeMode := NormalizeProbeMode(input.ProbeMode); probeMode != ProbeModeOff {
		...
		// Expand CIDRs / comma-lists into a concrete host slice. A
		// bare IP / hostname round-trips unchanged, so single-target
		// runs follow the same path as CIDR sweeps without any
		// special-casing.
		hosts, err := probe.ExpandTargets(target, probe.DefaultMaxExpandTargets)
		if err != nil {
			emit(fmt.Sprintf("[-] target expansion failed: %v", err))
			return RunResult{}, err
		}
		if len(hosts) > 1 {
			emit(fmt.Sprintf("[*] Expanded %s → %d hosts", target, len(hosts)))
		}
		for i, host := range hosts {
			...
			if len(hosts) > 1 {
				emit(fmt.Sprintf("[*] [%d/%d] Starting %s probe to %s...", i+1, len(hosts), depth, host))
			} else {
				emit(fmt.Sprintf("[*] Starting %s probe to %s...", depth, host))
			}
			summary, probeFindings := probe.RunProbeSuiteWithProgress(ctx, probeMode, input.ProbeRole, host, input.Duration, scoped, emit, func(ps ProbeSummary) {
				emitPartial(&ps)
			})
			cur := summary
			probeSummaries = append(probeSummaries, &cur)
			if len(probeFindings) > 0 {
				findings = append(findings, probeFindings...)
			}
		}
		if len(probeSummaries) > 0 {
			probeSummary = probeSummaries[0]
		}
		if err := ctx.Err(); err != nil {
			return RunResult{}, err
		}
	}
	findings = probe.NormalizeFindings(findings)
...
func LoadReport(path string) (Report, error) {
	...
	// Backfill the new Probes slice for reports written before
	// CIDR-sweep support landed. Older reports only persisted the
	// singular Probe; treat that as a one-target Probes slice so any
	// downstream code that wants to iterate finds it the same way.
	if len(report.Probes) == 0 && report.Probe != nil {
		report.Probes = []*ProbeSummary{report.Probe}
	}
	report.ReportLines = RenderReportLines(report)
	return report, nil
}
...
func RenderReportLines(report Report) []string {
	...
	probeSummary := report.Probe

	// ── Probe overview ─────────────────────────────────────────
	lines = append(lines, "")
	target := nonEmpty(strings.TrimSpace(probeSummary.Endpoint), "-")
	lines = append(lines, fmt.Sprintf("  target   %s", target))

	portsOpen := 0
	for _, pr := range probeSummary.PortResults {
		if pr.TunnelSuccess > 0 {
			portsOpen++
		}
	}
	totalPorts := len(probeSummary.Ports)
	if totalPorts == 0 {
		totalPorts = len(probe.DefaultProbePorts())
	}
	lines = append(lines, fmt.Sprintf("  ports    %d open / %d tested", portsOpen, totalPorts))

	if len(probeSummary.InternalRoutes) > 0 || len(probeSummary.InternetSubnets) > 0 {
		lines = append(lines, fmt.Sprintf("  routes   %d internal, %d internet", len(probeSummary.InternalRoutes), len(probeSummary.InternetSubnets)))
	}
	if probeSummary.AvgLatencyMs > 0 {
		lines = append(lines, fmt.Sprintf("  latency  ~%dms", probeSummary.AvgLatencyMs))
	}
	if probeSummary.TLSIntercepted {
		lines = append(lines, fmt.Sprintf("  tls      intercepted (%s)", probeSummary.TLSInterceptOrg))
	}

	// ── Tunnels ────────────────────────────────────────────────
	if probeSummary.TunnelAttempts > 0 {
		var carriers []string
		for _, m := range probeSummary.MethodResults {
			if m.TunnelAttempts <= 0 || !probe.IsCarrierTunnelMethod(strings.ToLower(strings.TrimSpace(m.Method))) {
				continue
			}
		...
		if probeSummary.DomainFrontingPossible {
			lines = append(lines, fmt.Sprintf("    - domain fronting via %s", probeSummary.DomainFrontingSNI))
		}
	}

	// ── Exfiltration ───────────────────────────────────────────
	if probeSummary.ExfilAttempts > 0 {
	...
	// ── Proxies ────────────────────────────────────────────────
	if len(probeSummary.Proxies) > 0 {
		lines = append(lines, "")
		lines = append(lines, fmt.Sprintf("  proxies  %d found, %d reachable", len(probeSummary.Proxies), probeSummary.ReachableProxyCount))
		for _, pline := range renderProxyCandidateLines(probeSummary.Proxies) {
			lines = append(lines, "    "+pline)
		}
	}

	// ── Services ───────────────────────────────────────────────
	if len(probeSummary.ServiceReachable) > 0 || len(probeSummary.ServiceBlocked) > 0 {
		lines = append(lines, "")
		lines = append(lines, "  services")
		if len(probeSummary.ServiceReachable) > 0 {
			lines = append(lines, wrapJoinLines("    reach  ", probeSummary.ServiceReachable, 72, "           ")...)
		}
		if len(probeSummary.ServiceBlocked) > 0 {
			lines = append(lines, wrapJoinLines("    block  ", probeSummary.ServiceBlocked, 72, "           ")...)
		}
	}
	...
	for _, sample := range samples {
		...
		if !ok {
			agg = &candidateAggregate{
				...
				services:      probe.NewServiceProfile(),
...
// each aggregate's probe.ServiceProfile.
func enrichAggregatesWithServices(ctx context.Context, aggs map[string]*candidateAggregate) {
	...
	resolutions := probe.ResolveExternalServices(ctx, allIPs, 400*time.Millisecond, 80)
...
func buildFindings(aggs map[string]*candidateAggregate) []Finding {
	...
	for _, agg := range aggs {
		...
		if agg.services != nil && agg.services.Total > 0 {
			...
			if cdnConns > 0 && (agg.writeB >= 50*1024*1024 || agg.role == "pivot" || agg.active) {
				sev := "watch"
				if agg.writeB >= 256*1024*1024 || agg.role == "pivot" {
					sev = "strong"
				}
			...
			for cat, count := range sp.Categories {
				if count > 0 && probe.ExfilCapableCategory(cat) {
					exfilCategories++
				}
			}
			if exfilCategories >= 2 {
				...
				for _, hit := range sp.SortedHits() {
					if probe.ExfilCapableCategory(hit.Category) {
	...
	return probe.NormalizeFindings(findings)
}
...
func renderProxyCandidateLines(endpoints []ProbeEndpoint) []string {
	...
	for _, endpoint := range endpoints {
		...
		case endpoint.PivotReachable:
			...
			rows = append(rows, row{
				...
				label:  hostPort + " -> " + nonEmpty(strings.TrimSpace(endpoint.PivotTarget), probe.DefaultProbePivotTarget) + " via " + via,
			})
	...
	sort.SliceStable(rows, func(i, j int) bool {
		scopeI := probe.EndpointScopeRank(rows[i].scope)
		scopeJ := probe.EndpointScopeRank(rows[j].scope)
		if scopeI != scopeJ {
...
func endpointDisplayTarget(endpoint ProbeEndpoint) string {
	...
	host, port, ok := probe.EndpointHostPort(endpoint.Endpoint)
	if !ok {
		return ""
	}
	return net.JoinHostPort(host, strconv.Itoa(port))
...
func buildEgressServiceSummary(aggs map[string]*candidateAggregate) *ServiceSummary {
	merged := make(map[string]*probe.ServiceHit, 32)
	...
	sort.SliceStable(entries, func(i, j int) bool {
		if probe.RiskRank(entries[i].Risk) != probe.RiskRank(entries[j].Risk) {
			return probe.RiskRank(entries[i].Risk) > probe.RiskRank(entries[j].Risk)
		}
...
func serviceNamesForCategory(sp *probe.ServiceProfile, category string) []string {
	if sp == nil {
		return nil
	}
