...
func sshRelay(rawConn, framedConn net.Conn) {
	const sshMsgChannelData byte = 94
	done := make(chan struct{}, 2)
	writeSSH := func(dst net.Conn, payload []byte) error {
		hdr := make([]byte, 9)
		hdr[0] = sshMsgChannelData
		binary.BigEndian.PutUint32(hdr[1:5], 0) // channel 0
		binary.BigEndian.PutUint32(hdr[5:9], uint32(len(payload)))
		dst.Write(hdr)
		_, err := dst.Write(payload)
		return err
	}
	readSSH := func(src net.Conn) ([]byte, error) {
		hdr := make([]byte, 9)
		if _, err := io.ReadFull(src, hdr); err != nil {
			return nil, err
		}
		...
		if length == 0 || length > 1<<20 {
			return nil, fmt.Errorf("invalid SSH data length")
		}
		data := make([]byte, length)
		if _, err := io.ReadFull(src, data); err != nil {
			return nil, err
	...
	// framedConn → rawConn: read SSH frames, write raw
	go func() {
		for {
			data, err := readSSH(framedConn)
			if err != nil {
				done <- struct{}{}
				return
			}
	...
	// rawConn → framedConn: read raw, write SSH frames
	go func() {
		buf := make([]byte, 16*1024)
		for {
			n, err := rawConn.Read(buf)
			...
			if writeSSH(framedConn, buf[:n]) != nil {
				done <- struct{}{}
				return
			}
		}
...
func serveSSHTunnel(ctx context.Context, port int, emit func(string)) tunnelResult {
	return serveTCPTunnel(ctx, "SSH", port, emit, func(conn net.Conn) (string, net.Conn, error) {
		...
		// Read custom SSH_MSG_CHANNEL_OPEN (0x5A) carrying target.
		// Format: [0x5A][4-byte length][target bytes]
		msgHdr := make([]byte, 5)
		if _, err := io.ReadFull(reader, msgHdr); err != nil {
			return "", nil, err
		}
		if msgHdr[0] != 0x5A {
			return "", nil, fmt.Errorf("expected SSH target message (0x5A), got 0x%02x", msgHdr[0])
		}
		targetLen := binary.BigEndian.Uint32(msgHdr[1:5])
		...
		remote, err := net.DialTimeout("tcp", target, 10*time.Second)
		if err != nil {
			// Send SSH_MSG_CHANNEL_OPEN_FAILURE (0x5C).
			conn.Write([]byte{0x5C, 0x00, 0x00, 0x00, 0x00})
			return "", nil, err
		}

		// Send SSH_MSG_CHANNEL_OPEN_CONFIRMATION (0x5B).
		conn.Write([]byte{0x5B, 0x00, 0x00, 0x00, 0x00})
		return target, remote, nil
	}, sshRelay)
}
...
func connectSSHTunnelClient(ctx context.Context, proxyAddr string, emit func(string)) tunnelResult {
	...
	return startLocalForwarder(ctx, proxyAddr, "ssh", emit, func(local net.Conn) {
		forwardGenericTextTunnel(local, proxyAddr, "ssh", emit, func(remote net.Conn, target string) bool {
			...
			// Send target in custom SSH_MSG_CHANNEL_OPEN (0x5A).
			targetBytes := []byte(target)
			msg := make([]byte, 5+len(targetBytes))
			msg[0] = 0x5A
			binary.BigEndian.PutUint32(msg[1:5], uint32(len(targetBytes)))
