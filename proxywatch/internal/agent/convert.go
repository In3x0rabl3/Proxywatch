...
func ToPBCandidate(c shared.Candidate) *pb.Candidate {
	...
	out.NamedPipes = append([]string(nil), c.NamedPipes...)
	out.Exited = c.Exited
	out.BeaconIntervalMs = clampInt32(c.BeaconIntervalMs)
	out.BeaconJitter = c.BeaconJitter
	return out
}
...
func FromPBCandidate(p *pb.Candidate, host string) shared.Candidate {
	...
	out.NamedPipes = append([]string(nil), p.NamedPipes...)
	out.Exited = p.Exited
	out.BeaconIntervalMs = int(p.BeaconIntervalMs)
	out.BeaconJitter = p.BeaconJitter
	return out
}
...
func ToPBProcess(p *shared.ProcessInfo) *pb.ProcessInfo {
	...
	return &pb.ProcessInfo{
		...
		SignatureTrust:       p.SignatureTrust,
		Signed:               p.Signed,
		Publisher:            p.Publisher,
		AuthenticodeOCSPSeen: p.AuthenticodeOCSPSeen,
		SHA256:               p.SHA256,
		PkgOwned:             p.PkgOwned,
		PkgOwnerName:         p.PkgOwnerName,
		PublisherDNSAligned:  p.PublisherDNSAligned,
		OnlineEvidence:       append([]string(nil), p.OnlineEvidence...),
	}
}
