...
func (s *Store) Update(host string, ts time.Time, cands []shared.Candidate) {
	...
	// Fire webhook alerts for newly promoted malicious candidates.
	alerts.ScanAndFire(host, cands)
}
...
func (r *RemoteScanner) Refresh(app *shared.AppState) {
	...
	if len(cands) > 0 {
		...
		for i := range cands {
			...
			// BUT preserve OS-reported listener state. Without this, system
			// services with outbound-* signals get rewritten to "outbound"
			// even when the kernel reports a bound port (the same bug fixed
			// in standalone classifier.go — was producing 15+ listener-as-
			// outbound FPs in server-mode telemetry on 2026-04-29).
			//
			// Also preserve the agent's malicious role when it classified
			// the candidate as suspicious with strong evidence. The agent
			// uses topology-based scoring (rank.go) which is more precise
			// than signal-only inference.
			if !mlAssigned {
				agentRole := c.Role
				agentSaysControl := shared.IsControlRole(agentRole)
				hasListenerNow := len(c.Listeners) > 0 || len(c.UDPListeners) > 0
				topologyRole := c.SuggestedRole
				if hasListenerNow && (topologyRole == "listen" || topologyRole == "listener") {
					c.Role = topologyRole
				} else if agentSaysControl && c.StrongEvidence {
					// Preserve agent's beacon/pivot decision with strong evidence.
					c.Role = agentRole
				} else {
					c.Role = ruleRole
				}
			...
			// ML-cannot-relax-suspicious gate (parity with standalone
			// classifier.go fix). When rank.go's topology decision was a
			// beacon-* role, ML may refine between suspicious roles but
			// must NOT escape to a benign role at any confidence. Without
			...
			// When the kernel reports a bound port AND rank.go said listen,
			// no ML or signal-vote may override to outbound/beacon-*.
			hasListenerNow := len(c.Listeners) > 0 || len(c.UDPListeners) > 0
			if hasListenerNow && (c.SuggestedRole == "listen" || c.SuggestedRole == "listener") &&
				c.Role != c.SuggestedRole {
				c.Role = c.SuggestedRole
			}

			// Role-stability revert: prevent beacon → outbound
			// flapping between beacon callbacks. Reads hist.LastRole
			// (set by the post-pass finalize at the end of this method)
			// to maintain the malicious role through the cooldown
			// window. The streak / LastRole COMMIT happens after the
			// cross-candidate post-passes (AggregateChildTunnelEvidence
			// + ApplyPivotLinger) below — capturing the truly-final
			// role for the cycle. Updating the streak here, before
			// those passes, made benign processes that get promoted to
			// pivot by ApplyPivotLinger flip every cycle and
			...
			// Shape-only beacon demotion — parity with standalone.
			if c.Proc != nil {
			...
			// the single source of truth for "preserve beacon-*/tunneling".
			...
			// Compare against DominantRole (statistical mode) for stable
			// processes so transient rule-engine flips don't degrade
			// the maturity ratio.
			if !c.MLActive {
				if profile := model.ResolveProfile(key); profile != nil {
					committed := ""
	...
	// Cross-candidate pivot promotion — parity with standalone
	// classifier.Classify(). Without this, the agent ships per-candidate
	// scores but the SSH-D-style pattern (parent listener + child session
	// handler doing SOCKS forwarding) never gets correlated, so sshd's
	// session handler stays at role=outbound forever even when the pivot
	// signal fires. AggregateChildTunnelEvidence promotes the parent
	// listener; ApplyPivotLinger pins the relay-child as pivot
	// for 60s. We synthesize a processes map from the candidates' own
	// Proc fields so the 4-deep ancestor walk can traverse intermediate
	// privsep helpers that ARE shipped as candidates.
	processes := make(map[int]*shared.ProcessInfo, len(cands))
	for i := range cands {
		if cands[i].Proc != nil {
			processes[cands[i].Proc.Pid] = cands[i].Proc
		}
	}
	scoring.AggregateChildTunnelEvidence(cands, processes, now)
	scoring.ApplyPivotLinger(cands, processes, now)

	// Final role-streak commit. Captures the TRULY-final role for the
	// cycle (post AggregateChildTunnelEvidence + ApplyPivotLinger).
	// Mirrors finalizeRoleStreaks() in detection/classifier.go for
	// parity. Updating the streak in the per-candidate loop (before
	// these passes) caused processes that get promoted to pivot
	// by ApplyPivotLinger to flip every cycle, resetting the streak
	// and trapping the candidate in `analyzing` forever. See
	// CandidateState in shared/candidate.go for the streak threshold.
	for i := range cands {
		c := &cands[i]
		if c.Proc == nil {
			continue
		...
		if hist == nil {
			...
			shared.ProcHistoryByPID[c.Proc.Pid] = hist
		}
		// Compare by role family (parity with classifier.finalizeRoleStreaks):
		// listen ↔ listener etc. shouldn't reset the streak.
