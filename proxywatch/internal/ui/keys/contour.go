...
func HandleContourModeKey(app *shared.AppState, tev *tcell.EventKey) bool {
	if app.ContourNewShowMenu {
		...
		case tcell.KeyEnter:
			if app.ContourNewMenuIndex >= 0 && app.ContourNewMenuIndex < len(app.ContourNewMenuOptions) {
				...
				case "direction":
					if choice == "true" {
						app.ContourNewDirection = "Reverse"
					} else {
						app.ContourNewDirection = "Forward"
					}
				case "service":
	...
	// Fields: 0=Role, 1=Mode, 2=Service, 3=Method, 4=Reverse, 5=Action
	if app.ContourNewMode == "Services" {
		const servicesMaxField = 5
		switch tev.Key() {
		case tcell.KeyUp:
		...
		case tcell.KeyEnter:
			...
			case 0: // Role
				...
				app.ContourNewMenuTitle = "Select Role"
			...
			case 1: // Mode
				app.ContourNewShowMenu = true
				app.ContourNewMenuKind = "mode"
				app.ContourNewMenuTitle = "Select Mode"
				app.ContourNewMenuOptions = []string{"Tunnel", "Services", "Scan"}
				idx := 0
				for i, o := range app.ContourNewMenuOptions {
					if o == app.ContourNewMode {
						idx = i
					}
				}
				app.ContourNewMenuIndex = idx
			case 2: // Service
				if len(app.ContourNewServices) > 0 {
					...
					app.ContourNewMenuTitle = "Select Service"
			...
			case 3: // Method
				...
				app.ContourNewMenuTitle = "Select Method"
			...
			case 4: // Reverse
				...
				app.ContourNewMenuTitle = "Reverse Tunnel"
				app.ContourNewMenuOptions = []string{"false", "true"}
				idx := 0
	...
	case tcell.KeyEnter:
		...
		// Field order: 0=Role, 1=Mode, 2=Protocol, 3=Port, 4=Reverse, 5=Action
		switch app.ContourNewField {
		case 0: // Role
			app.ContourNewShowMenu = true
			app.ContourNewMenuKind = "role"
		...
		case 2: // Protocol
			if len(app.ContourNewMethods) > 0 {
				...
				app.ContourNewMenuTitle = "Select Protocol"
		...
		case 3: // Port
			...
			if len(ports) > 0 {
				opts := make([]string, 0, len(ports)+1)
				opts = append(opts, "auto")
				for _, p := range ports {
					opts = append(opts, fmt.Sprintf("%d", p))
				}
				app.ContourNewShowMenu = true
				app.ContourNewMenuKind = "port"
				app.ContourNewMenuTitle = "Select Port"
				app.ContourNewMenuOptions = opts
				idx := 0
				if app.ContourNewPort > 0 {
					for i, o := range opts {
						if o == fmt.Sprintf("%d", app.ContourNewPort) {
							idx = i
						}
					}
				}
...
func ContourNewMaxField(mode, role string) int {
	...
	return 5 // Role, Protocol, Port, Mode, Reverse, Action
}
...
func StartContourTunnel(app *shared.AppState) {
	...
	setWorkflowStatus(app, &app.ContourStatusText, &app.ContourStatusError, &app.ContourStatusUntil,
		fmt.Sprintf("%s tunnel starting...", role), false)
}
