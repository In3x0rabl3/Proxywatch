...
var (
	RoleMenuChoices    = []string{"recommended", "all", "beacon", "pivot", "listener", "outbound"}
...
func applyRolePreset(app *shared.AppState, preset string) {
	...
	switch preset {
	case "recommended":
		app.RoleFilterOverride = shared.ParseRoleFilter("session,beacon,pivot")
	case "all":
...
func JumpToWorkflow(app *shared.AppState, r rune) bool {
	...
	case '1':
		target = shared.ModePcapAnalyzer
	case '2':
