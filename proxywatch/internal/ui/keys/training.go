...
func handleTrainingAction(app *shared.AppState) bool {
	...
	case TrainingFieldForceML:
		// Toggle force-qualify ML model
		if model.MLForceQualified() {
			model.UnforceQualifyML()
			shared.LogInfo("model", "ML force-qualification removed — reverting to rule agreement check")
		} else {
			model.ForceQualifyML()
			shared.LogInfo("model", "ML force-qualified — trusting ML predictions over rules")
		}
	}
