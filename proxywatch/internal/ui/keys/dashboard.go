...
func HandleDashboardKey(app *shared.AppState, tev *tcell.EventKey) bool {
	if app.ShowHelp || app.ShowRoleMenu || app.ShowSortMenu || app.ShowRefreshMenu || app.ShowWhitelistPanel {
	...
	case 'o', 'O':
		EnterContourMode(app)
	case 'w':
		app.ShowWhitelistPanel = !app.ShowWhitelistPanel
	case 'q':
...
func HandleDashboardOverlayKey(app *shared.AppState, tev *tcell.EventKey) bool {
	...
	// Whitelist panel navigation
	if app.ShowWhitelistPanel {
		switch tev.Key() {
		case tcell.KeyUp:
	...
	case 'b', 'B':
		...
		return false
	case 'w':
		// Toggle whitelist panel overlay
		if app.ShowWhitelistPanel {
			app.ShowWhitelistPanel = false
		} else {
			CloseDashboardOverlays(app)
			app.ShowWhitelistPanel = true
		}
		return false
	case 'k', 'K':
		CloseDashboardOverlays(app)
...
func DashboardOverlayOpen(app *shared.AppState) bool {
	return app.ShowHelp || app.ShowRoleMenu || app.ShowSortMenu || app.ShowRefreshMenu || app.ShowWhitelistPanel
}
...
func rolePresetOptions() []string {
	return []string{"recommended", "all", "beacon", "pivot", "listener", "outbound"}
}
