...
const (
	...
	TrainingFieldReset
	TrainingFieldForceML
)

const TrainingFieldMax = TrainingFieldForceML
...
const (
	...
	trainingFieldForceML   = TrainingFieldForceML
...
func renderAccentReportPanel(opts ReportPanelOpts) string {
	return common.RenderAccentReportPanel(opts)
}
...
func pcapAnalyzerMenuHelpOptions() []string { return common.PcapAnalyzerMenuHelpOptions() }
...
var (
	...
	HandlePcapAnalyzerKey         func(*shared.AppState, *tcell.EventKey) bool
	EnterPcapAnalyzerMode         func(*shared.AppState)
)
...
var (
	...
	// lgSelectCursor is the canonical style for the "> " cursor
	// glyph on a selected row across every dashboard view (pcap
	// FINDINGS, PROCESS VIEW, HOST VIEW, etc). Bold + accent
	// foreground over the selection background so the cursor pops
	// inside the highlighted bar. Centralised so visual drift
	// between views can't happen.
	lgSelectCursor = lipgloss.NewStyle().Bold(true).
			Foreground(common.ColorAccent).
			Background(common.ColorSelect)
)

func lgRoleStyle(role string) lipgloss.Style {
	// Match main dashboard role styling
