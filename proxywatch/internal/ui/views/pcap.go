...
type PcapAnalyzerModel struct {
	...
	// stickyTimelineEvents is the cumulative list of (lane, timestamp)
	// activity samples observed during the current pcap session. We
	// store TIMESTAMPS (not pre-computed cell indices) because the
	// time axis grows as the pcap captures more — a fixed-cell matrix
	// would silently mis-map old cells to wrong times when the axis
	// expands. Each render projects all events onto the current axis,
	// so cells from cycle N stay anchored to their correct moment in
	// time even after cycle N+M's axis is wider.
	stickyTimelineEvents []timelineSample
...
// timelineSample records a single (lane, timestamp) activity sample
// the timeline bar should light. We store TIMESTAMPS rather than
// pre-computed cell indices so axis growth (PcapEnd marching forward
// in tail mode) doesn't silently mis-map old cells. Each render
// projects all samples onto the current axis range, keeping
// previously-lit moments anchored to their actual time.
...
func (m PcapAnalyzerModel) Update(msg tea.Msg) (PcapAnalyzerModel, tea.Cmd) {
	...
	case PcapAnalysisResultMsg:
		...
		m.findingsParamShowAll = m.app.PcapShowAllFindings
		// Timeline events MUST be collected and merged here in Update
		// (pointer return), not inside View() (value receiver — any
		// mutation to m there is discarded). Append every cycle's
		// events into the session-wide sticky log so the timeline
		// bars only grow.
		newEvents := collectTimelineEvents(&res, m.findings, m.tunneling)
		m.stickyTimelineEvents = mergeTimelineSamples(m.stickyTimelineEvents, newEvents)
		m.RefreshContent()
...
// renderTimelineStrip draws the per-window severity strip across the
// top of the results panel. Each cell is one replay window; cells are
// bucket-aggregated when there are more windows than terminal columns.
// Colors:
//
//	dim teal  — quiet (only outbound / no candidates)
//	yellow    — analyzing / beacon observed
//	orange    — pivot observed (relay topology found)
//	red       — actively tunneling (data flowing through a pivot)
//
// Returns just the colored bar (one line). Used by renderTimelinePanel
// to build the full bordered timeline frame.
func renderTimelineStrip(res *pcap.IngestResult, width int) string {
	if res == nil || len(res.Timeline) == 0 || width < 16 {
		return ""
	}
	stripW := width
	if stripW < 16 {
	...
	// Bucket-aggregate when windows > stripW (max severity per bucket).
	values := make([]uint8, stripW)
	if len(res.Timeline) <= stripW {
		// Stretch left-aligned; tail empty.
		for i, tw := range res.Timeline {
			if i >= stripW {
				break
			}
			values[i] = tw.Severity
		}
	} else {
		bucketSize := float64(len(res.Timeline)) / float64(stripW)
		for cell := 0; cell < stripW; cell++ {
			start := int(float64(cell) * bucketSize)
			end := int(float64(cell+1) * bucketSize)
			if end > len(res.Timeline) {
				end = len(res.Timeline)
			}
			var maxSev uint8
			for j := start; j < end; j++ {
				if res.Timeline[j].Severity > maxSev {
					maxSev = res.Timeline[j].Severity
				}
			}
			values[cell] = maxSev
		}
	}
	colors := timelineSeverityColors()
	var sb strings.Builder
	for _, sev := range values {
		c := colors[0]
		if int(sev) < len(colors) {
			c = colors[sev]
		}
...
// timelineLanes is the bar's 4-layer ordering — one row per
// category, each rendered as a thin horizontal line that doesn't
// touch the lines above or below it. Used both for rendering and
// stats computation — a single source of truth keeps the legend,
// stats, and bar in lockstep.
type timelineLane struct {
	role  string // role-family string; "tunneling" is special-cased via the tunneling[] flag
	color lipgloss.Color
}

func timelineLanes() []timelineLane {
	palette := timelineSeverityColors()
	return []timelineLane{
		{role: "tunneling", color: palette[4]}, // red
		{role: "pivot", color: palette[3]},     // gold
		{role: "beacon", color: palette[2]},    // pink
		{role: "outbound", color: colorCyan},
	}
}

// timelineAxisRange returns the time anchors the bar's cell grid
// should map to. Anchored to the FULL capture span (PcapStart →
// PcapEnd) so the bar's scale doesn't shift as new windows get
// classified mid-replay — already-lit cells stay in place. Falls
// back to Timeline[0]/Timeline[last] when PcapStart/PcapEnd aren't
// populated yet (very early in ingest, before walkPcap finishes).
func timelineAxisRange(res *pcap.IngestResult) (start, end time.Time) {
	if res == nil {
		return time.Time{}, time.Time{}
	}
	start = res.PcapStart
	end = res.PcapEnd
	if start.IsZero() && len(res.Timeline) > 0 {
		start = res.Timeline[0].WindowEndAt
	}
	if end.IsZero() && len(res.Timeline) > 0 {
		end = res.Timeline[len(res.Timeline)-1].WindowEndAt
	}
	return start, end
}

// computeTimelineMatrix builds a per-lane × per-cell activity matrix
// from the FINDINGS list (post-filter). Each finding projects its
// [FirstPacket, LastPacket] range onto the cell grid and marks every
// overlapping cell active in its lane.
//
// Bar time axis is fixed to the FULL CAPTURE span (PcapStart →
// PcapEnd) — not the classified-so-far range. Without this, every
// new window classified during paced replay would shift the bar's
// scale and previously-lit cells would visually compress and "move"
// left, making the timeline feel unstable. With a fixed axis, a
// finding's cell range stays in place from the moment it appears
// and progressively more cells light up to the right as replay
// reaches new findings — the bar grows in painted area, not in
// scale. Already-painted cells stay painted (the historical record
// of activity) even after that role's traffic stops.
//
// Lane order matches timelineLanes(): 0=tunneling, 1=pivot, 2=channel,
// 3=outbound. tunneling[i] flag makes finding i light up the
// tunneling lane independently of its role family.
//
// Findings whose PID has no FirstPacketByPID entry (per-flow PIDs
// without time data — see ingest.go's addEndpointBytes) used to fall
// back to the full pcap span, which lit the entire bar erroneously.
// We now SKIP such findings — they have no usable time anchor so
// they can't contribute to the temporal layout.
// collectTimelineEvents extracts (lane, timestamp) activity samples
// from the current cycle's findings list. Same per-finding logic as
// computeTimelineMatrix (anchor at FirstPacketByPID + above-floor
// PerWindowBytesByPID windows + tunneling lane echo) — but emits
// timestamps instead of cell indices so events can be re-projected
// onto whatever axis the next render uses.
func collectTimelineEvents(res *pcap.IngestResult, findings []shared.Candidate, tunneling []bool) []timelineSample {
	if res == nil || len(findings) == 0 {
		return nil
	}
	startT, _ := timelineAxisRange(res)
	const perWindowFloor uint64 = 512
	out := make([]timelineSample, 0, len(findings)*4)
	for i, c := range findings {
		...
		isTunneling := i < len(tunneling) && tunneling[i]
		anchorAt := res.FirstPacketByPID[c.Proc.Pid]
		if anchorAt.IsZero() {
			anchorAt = res.LastPacketByPID[c.Proc.Pid]
		}
		if !anchorAt.IsZero() {
			out = append(out, timelineSample{lane: lane, at: anchorAt})
			if isTunneling {
				out = append(out, timelineSample{lane: 0, at: anchorAt})
			}
		}
		series := res.PerWindowBytesByPID[c.Proc.Pid]
		for wi, bytes := range series {
			if bytes < perWindowFloor {
				continue
			}
			windowEnd := startT.Add(time.Duration(wi+1) * pcap.Window)
			out = append(out, timelineSample{lane: lane, at: windowEnd})
			if isTunneling {
				out = append(out, timelineSample{lane: 0, at: windowEnd})
			}
		}
	}
	return out
...
// mergeTimelineSamples appends every event in `add` to `dst` unless an
// identical (lane, timestamp) pair is already present. Dedup uses a
// struct-keyed map (avoiding the lane<<60 | UnixNano scheme — UnixNano
// already crosses 2^60 in current dates, so the lane bits would
// collide with the high bits of the timestamp).
func mergeTimelineSamples(dst, add []timelineSample) []timelineSample {
	if len(add) == 0 {
		return dst
	}
	type key struct {
	...
	seen := make(map[key]struct{}, len(dst)+len(add))
	for _, s := range dst {
		seen[key{lane: s.lane, nanos: s.at.UnixNano()}] = struct{}{}
	}
	for _, s := range add {
		k := key{lane: s.lane, nanos: s.at.UnixNano()}
		if _, ok := seen[k]; ok {
			continue
		}
		seen[k] = struct{}{}
...
// projectTimelineEvents rebuilds a lane × cell matrix from the sticky
// event log, using the CURRENT axis range. Cells previously lit at
// timestamp T stay anchored to T even after the axis grows (in tail
// mode PcapEnd advances every cycle); only the rightmost cells shift
// further right as new windows arrive. Events whose timestamp falls
// outside the current axis are clamped to the nearest edge.
func projectTimelineEvents(events []timelineSample, res *pcap.IngestResult, width int) [][]bool {
	lanes := timelineLanes()
	matrix := make([][]bool, len(lanes))
	for i := range matrix {
		matrix[i] = make([]bool, width)
	}
	if res == nil || width <= 0 || len(events) == 0 {
		return matrix
	}
	startT, endT := timelineAxisRange(res)
	if !endT.After(startT) {
		endT = startT.Add(time.Second)
	}
	span := endT.Sub(startT)
	for _, ev := range events {
		if ev.lane < 0 || ev.lane >= len(matrix) {
			continue
		}
		var cell int
		switch {
		case ev.at.Before(startT):
			cell = 0
		case !ev.at.Before(endT):
			cell = width - 1
		default:
			frac := float64(ev.at.Sub(startT)) / float64(span)
			cell = int(frac * float64(width))
			if cell < 0 {
				cell = 0
			}
			if cell >= width {
		...
		matrix[ev.lane][cell] = true
	}
	return matrix
}

func computeTimelineMatrix(res *pcap.IngestResult, findings []shared.Candidate, tunneling []bool, width int) [][]bool {
	lanes := timelineLanes()
	matrix := make([][]bool, len(lanes))
	for i := range matrix {
		matrix[i] = make([]bool, width)
	}
	if res == nil || len(res.Timeline) == 0 || width <= 0 {
		return matrix
	}
	startT, endT := timelineAxisRange(res)
	if !endT.After(startT) {
	...
	span := endT.Sub(startT)
	timeToCell := func(t time.Time) int {
		if t.Before(startT) {
			return 0
		}
		if !t.Before(endT) {
		...
		frac := float64(t.Sub(startT)) / float64(span)
		c := int(frac * float64(width))
		if c < 0 {
			c = 0
		}
		if c >= width {
	...
	// Per-finding per-window cell marking + FirstPacket anchor.
	// For each kept finding we walk its PerWindowBytesByPID series
	// and mark cells where THAT finding's own bytes-in-window cross
	// perWindowFloor (so TCP keepalives don't paint cells just
	// because the socket stayed ESTABLISHED). The anchor cell at
	// FirstPacket is unconditional so even small findings with no
	// above-floor windows still have visible presence on the bar.
	//
	// 1:1 invariant: every lit cell traces to a kept finding row —
	// either at its FirstPacket anchor or at a window where its
	// own bytes flowed. Findings sorted by time means scanning the
	// table top-to-bottom matches the bar reading left-to-right.
	const perWindowFloor uint64 = 512
	...
	for i, c := range findings {
		...
		if !anchorAt.IsZero() {
			cell := timeToCell(anchorAt)
			matrix[lane][cell] = true
			if isTunneling {
				matrix[0][cell] = true
			}
		}
...
// downsampleMatrix collapses a high-resolution lane × cell matrix
// down to `dst` cells per lane. A destination cell is lit when ANY
// of the source cells that map into it are lit. Caller invariant:
// every source matrix lane has the same length (computeTimelineMatrix
// always returns rectangular matrices).
//
// Used to render the bar at the operator's available terminal width
// while keeping the activity density tied to the high-resolution
// stats matrix — so the visible bar coverage and the percentage
// numbers always agree, regardless of how wide the terminal is.
func downsampleMatrix(src [][]bool, dst int) [][]bool {
	if dst <= 0 || len(src) == 0 {
		return nil
	}
	out := make([][]bool, len(src))
	for li := range src {
		out[li] = make([]bool, dst)
	}
	srcW := 0
	if len(src) > 0 {
		srcW = len(src[0])
	...
	if dst >= srcW {
		// Destination is wider than source — nothing to fold. Copy
		// the leading srcW cells; trailing dst-srcW stay dark.
		for li := range src {
			copy(out[li], src[li])
		}
		return out
	}
	// Each destination cell aggregates a [start, end) slice of the
	// source. Compute the slice via integer math so cells are
	// evenly distributed.
	for j := 0; j < dst; j++ {
		start := j * srcW / dst
		end := (j + 1) * srcW / dst
...
// renderTimelineMultiBar produces the multi-layer timeline as 4
// stacked rows of thin horizontal lines — one row per category. Each
// line uses the box-drawing horizontal `━` (HEAVY HORIZONTAL) glyph;
// adjacent rows leave empty space between them so colors don't
// touch above or below. Active cells render in the lane's color;
// inactive cells render dim so the bar still reads as a continuous
// track per lane.
//
// Each row is prefixed with `<labelPrefix>` (e.g. "tunneling 0.1% ")
// — the per-lane label + percentage rendered in dim text. Indices
// must match timelineLanes() order:
//
//	row 1 — tunneling (red)
//	row 2 — pivot     (gold)
//	row 3 — channel   (pink)
//	row 4 — outbound  (cyan)
//
// Listener is intentionally excluded; it's host-context that doesn't
// belong in the activity timeline.
func renderTimelineMultiBar(matrix [][]bool, width int, labelPrefixes []string) string {
	if width < 16 || len(matrix) == 0 {
		return ""
	}
	palette := timelineSeverityColors()
	idle := palette[0]
	...
	for li, ln := range lanes {
		if li < len(labelPrefixes) {
			rows.WriteString(labelPrefixes[li])
		}
		for cell := 0; cell < width && cell < len(matrix[li]); cell++ {
			fg := idle
			if matrix[li][cell] {
				fg = ln.color
			}
			rows.WriteString(lipgloss.NewStyle().Foreground(fg).Render("━"))
		}
		if li < len(lanes)-1 {
			rows.WriteString("\n")
		}
...
// formatTimelineRange returns "start  →  end" for the pcap time-shifted
// span (HH:MM:SS) so the timeline strip has clear endpoints.
func formatTimelineRange(res *pcap.IngestResult) string {
	if len(res.Timeline) == 0 {
		return ""
	}
	first := res.Timeline[0].WindowEndAt
	last := res.Timeline[len(res.Timeline)-1].WindowEndAt
	return first.Format("15:04:05") + "   →   " + last.Format("15:04:05")
}

// renderTimelinePanel draws the bordered TIMELINE panel:
//
//	row 1: cursor mark (▼) above the bar cell where the selected
//	       finding's first packet lives
//	rows 2-5: four thin-line bar rows (tunneling/pivot/channel/outbound)
//	row 6: percentages legend, colored to match the bar so the line
//	       doubles as a color key
//
// `selected` is the candidate currently under the Findings cursor; its
// FirstPacket → window-cell maps to the cursor mark above the bar.
func renderTimelinePanel(res *pcap.IngestResult, w int, selected *shared.Candidate, findings []shared.Candidate, tunneling []bool, stickySamples *[]timelineSample) string {
	if w < 24 {
		return ""
	}
	const sidePad = 2
	innerW := w - sidePad*2 - 2 // account for left + right panel borders + side padding
	if innerW < 16 {
		return ""
	}
	// Empty placeholder — render an empty TIMELINE box from the
	// moment the pcap analyzer view opens so the operator sees the
	// layout structure right away. Filled with bar + stats once
	// ingest produces Timeline data.
	if res == nil || len(res.Timeline) == 0 {
		return renderAccentReportPanel(ReportPanelOpts{
			Title:   "TIMELINE",
			Width:   w,
			Height:  timelinePanelHeight,
			Content: dimText.Render("  Awaiting capture data..."),
		})
	}
	// Per-role-family percentages. Computed from a FIXED-RESOLUTION
	// stats matrix (statsCellResolution cells), NOT the display
	// matrix at barW. Reason: the percentage of "% of capture time
	// this lane was active" is a property of the data, not of the
	// terminal. Different terminal widths must produce identical
	// numbers.
	//
	// Sticky cumulative behaviour: the session-wide event log is
	// merged in Update's PcapAnalysisResultMsg handler (the only
	// place state mutation persists — View has a value receiver and
	// any writes here would be discarded with the local copy). View
	// only PROJECTS the merged events onto the current axis to build
	// the matrix.
	var events []timelineSample
	if stickySamples != nil {
		events = *stickySamples
	}
	if len(events) == 0 {
		// Defensive fallback for edge timing where View runs before
		// the first PcapAnalysisResultMsg has populated the sticky log
		// — recompute from the current findings so the bar isn't
		// blank during the very first render.
		events = collectTimelineEvents(res, findings, tunneling)
	}
	statsMatrix := projectTimelineEvents(events, res, statsCellResolution)
	total := statsCellResolution
	var tun, piv, ch, out int
	for cell := 0; cell < total; cell++ {
		if statsMatrix[0][cell] {
			tun++
		}
		if statsMatrix[1][cell] {
			piv++
		}
		if statsMatrix[2][cell] {
			ch++
		}
		if statsMatrix[3][cell] {
			out++
		}
	}
	// pctStr formats lane coverage. Below 1% we render with 1 decimal
	// so single-cell anchors don't round to 0 and contradict the
	// visible bar.
	pctStr := func(n int) string {
		if total == 0 || n == 0 {
			return "0%"
		}
		raw := float64(n) * 100.0 / float64(total)
		if raw < 1.0 {
			return fmt.Sprintf("%.1f%%", raw)
		}
		return fmt.Sprintf("%d%%", int(raw+0.5))
	}

	// Per-lane label prefix: "<name>  <pct>" rendered in dim text
	// (no per-lane colour — the operator can already tell lanes
	// apart by row position + bar colour). Rendered to the LEFT of
	// each bar so the eye can read label and activity on the same
	// horizontal line. Padded to a uniform width so all bars start
	// at the same column.
	const (
		laneNameW = 9 // "tunneling" / "outbound" — longest is 9 chars
		lanePctW  = 5 // " 100%" / " 0.1%"
	)
	pcts := []string{pctStr(tun), pctStr(piv), pctStr(ch), pctStr(out)}
	names := []string{"tunneling", "pivot", "channel", "outbound"}
	labelPrefixes := make([]string, len(names))
	labelW := laneNameW + 1 + lanePctW + 2 // name + space + pct + 2-space gutter to bar
	for i := range names {
		labelPrefixes[i] = dimText.Render(fmt.Sprintf(
			"%-*s %*s  ", laneNameW, names[i], lanePctW, pcts[i],
		))
	}
	// Bar gets whatever's left of innerW after the label prefix.
	barW := innerW - labelW
	if barW < 16 {
		barW = 16
	}

	// Display matrix is downsampled from the high-resolution stats
	// matrix so the bar's visible density matches the percentages
	// (both come from the SAME source). Recomputing the matrix at
	// barW would change the cell-to-time mapping — bigger cells
	// fold more windows together, more cells cross perWindowFloor,
	// the bar looks solid even when the percentage is low.
	matrix := downsampleMatrix(statsMatrix, barW)
	barRows := renderTimelineMultiBar(matrix, barW, labelPrefixes)

	// Cursor row: a ▼ above the bar cell where the selected finding's
	// first packet lives. Indented by labelW so the marker aligns
	// with the bar cells (not the labels).
	cursorRow := strings.Repeat(" ", innerW)
	if selected != nil && selected.Proc != nil && len(res.Timeline) > 0 {
		if firstPkt, ok := res.FirstPacketByPID[selected.Proc.Pid]; ok && !firstPkt.IsZero() {
			startT, endT := timelineAxisRange(res)
			if !endT.After(startT) {
				endT = startT.Add(time.Second)
			}
			frac := float64(firstPkt.Sub(startT)) / float64(endT.Sub(startT))
			if frac < 0 {
				frac = 0
			}
			if frac > 1 {
			...
			marker := lipgloss.NewStyle().Foreground(colorAccent).Bold(true).Render(platform.IconCursorDown)
			cursorRow = strings.Repeat(" ", labelW+col) + marker + strings.Repeat(" ", barW-col-1)
		}
	}

	// Inner content layout (top → bottom):
	//   1. Cursor row (▼ over selected finding's first packet,
	//      indented to align with the bar cells past the label
	//      prefix column)
	//   2-5. Four thin-line bar rows, each prefixed with
	//        "<name>  <pct>" in dim text (the legend rendered
	//        in-line with the bar instead of below it).
	pad := strings.Repeat(" ", sidePad)
	var content strings.Builder
	content.WriteString(pad + cursorRow + "\n")
	for _, row := range strings.Split(barRows, "\n") {
		content.WriteString(pad + row + "\n")
	}
...
// timelinePanelHeight is the fixed total height of the TIMELINE
// panel: 2 borders + 5 content lines (cursor row + 4 thin-line bar
// rows). Lane labels + percentages live INSIDE each bar row now
// (rendered as a dim "<name>  <pct>" prefix), so no separate stats
// line at the bottom.
const timelinePanelHeight = 7

// statsCellResolution is the fixed cell count used to compute the
// tunneling/pivot/channel/outbound percentages on the timeline
// panel. Independent of terminal width so the same pcap analysed
// on different machines produces identical numbers. 1000 cells is
// fine-grained enough that anchor-cell rounding (each finding
// covers ≥1 cell) doesn't dominate the percentage.
const statsCellResolution = 1000

// timelineSeverityColors returns the 5-tier color palette for the
// timeline bar AND the stats legend, in severity order:
//
//	0 quiet (no candidates / outbound only)
//	1 analyzing
//	2 beacon observed
//	3 pivot observed
//	4 tunneling actively flowing
//
// Palette is sourced from common.styles.go so the bar's cells match
// the role / state colors used by the findings table and the rest of
// the dashboard. Without this, beacon cells looked orange in
// the bar but pink in the table — visually inconsistent.
func timelineSeverityColors() []lipgloss.Color {
	return []lipgloss.Color{
		// Neutral dark grey baseline — was previously "#2A3A2F"
		// which has a green tint (G=58 vs R=42, B=47) that
		// rendered noticeably green on Windows console fonts. Pure
		// grey #3A3A3A renders consistently as dim grey across
		// Linux + Windows terminals.
		lipgloss.Color("#3A3A3A"), // 0 — quiet baseline (neutral dim grey)
		colorDim,                  // 1 — analyzing (dim)
		colorSession,              // 2 — beacon (pink, matches table)
		colorWarn,                 // 3 — pivot (gold, matches table)
		lipgloss.Color("#E04050"), // 4 — tunneling (saturated red — distinct from sev-2 pink so the bar still differentiates)
	}
}

// truncateMidIfNeeded truncates s to width with an ellipsis in the
// middle if it overflows. No-op when s already fits.
func truncateMidIfNeeded(s string, width int) string {
...
func pcapDestEndpoint(c shared.Candidate) string {
	role := strings.ToLower(c.Role)
	...
	for _, conn := range c.Conns {
		...
		ep := fmt.Sprintf("%s:%d", conn.RemoteAddress, conn.RemotePort)
		isInternal := shared.IsInternalIP(conn.RemoteAddress)
		isExternal := !isInternal &&
			!shared.IsLoopbackIP(conn.RemoteAddress) &&
			!shared.IsWildcardIP(conn.RemoteAddress)
		cs = append(cs, cand{
			ep:          ep,
			established: strings.EqualFold(conn.State, "ESTABLISHED"),
			internal:    isInternal,
			external:    isExternal,
		})
	}
	...
	sort.SliceStable(cs, func(i, j int) bool { return cs[i].ep < cs[j].ep })

	pick := func(filter func(cand) bool) string {
		for _, x := range cs {
			if filter(x) {
				return x.ep
			}
	...
	switch role {
	case "pivot":
		// Pivots forward inbound to an internal target — that target is
		// the operator-relevant peer. Prefer established + internal.
		if ep := pick(func(x cand) bool { return x.established && x.internal }); ep != "" {
			return ep
		}
		if ep := pick(func(x cand) bool { return x.internal }); ep != "" {
			return ep
		}
	case "beacon":
		// C2 callbacks go external. Prefer established + external.
		if ep := pick(func(x cand) bool { return x.established && x.external }); ep != "" {
			return ep
		}
		if ep := pick(func(x cand) bool { return x.external }); ep != "" {
			return ep
		}
	}
	if ep := pick(func(x cand) bool { return x.established }); ep != "" {
		return ep
	}
	return cs[0].ep
}

// tunnelStyle highlights the TUNNEL column when the candidate is
// actively forwarding (✓), otherwise renders dimmed empty space.
func tunnelStyle(active bool) lipgloss.Style {
	if active {
...
// truncatePathLeft trims a path from the left with a leading "…" when
// it overflows w. Used so the pcap filename is always visible even when
// the parent directories are deep.
...
func (m PcapAnalyzerModel) renderReportPanel() string {
	w := m.width
	if w <= 0 {
		w = 80
	}
	...
	case pcapStageResults:
		if m.tailing {
			panelTitle = "WATCHING ● live"
		} else {
			panelTitle = "PACKET VIEW"
		}
	}
...
func pcapKv(label, value string) string {
	return dimText.Render(fmt.Sprintf("  %-15s ", label)) + bodyText.Render(value)
}

// formatNumber returns n with thousands separators (US style) so large
// packet counts read at a glance: 8335 → "8,335", 3188000 → "3,188,000".
