...
func (m WhitelistModel) renderBottomBar(w int) string {
	line := bgSp(1) + dimText.Render("esc dashboard    ↑↓ select    enter whitelist    ? menu")
...
func (m WhitelistModel) processListHeight(totalH int) int {
	available := totalH - shellBannerHeight(m.width)
	if available < 8 {
