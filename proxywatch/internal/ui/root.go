...
func NewRootModel(app *shared.AppState, scanner shared.Scanner) RootModel {
	return RootModel{
		...
		pcap:       views.NewPcapAnalyzerModel(app),
...
func (m RootModel) Update(msg tea.Msg) (retModel tea.Model, retCmd tea.Cmd) {
	...
	// Same pattern for pcap analyzer: when an analysis (or watch
	// session) is running and the operator has navigated to another
	// view, keep the pcap model receiving non-key messages so its
	// progress / result listeners stay armed and findings + timeline
	// keep updating in the background. Coming back to the pcap view
	// shows the latest state.
	if m.app.Mode != shared.ModePcapAnalyzer && m.app.PcapAnalysisRun {
		if _, isKey := msg.(tea.KeyMsg); !isKey {
			var cmd tea.Cmd
			m.pcap, cmd = m.pcap.Update(msg)
			if cmd != nil {
				cmds = append(cmds, cmd)
			}
		}
