...
func CollectLiveLines(app *shared.AppState) []string {
	...
	// Format as tactical time T+MM:SS
	elapsedMin := int(elapsed.Seconds()) / 60
	elapsedSec := int(elapsed.Seconds()) % 60

	// Check if running indefinitely (zero time)
	if app.CollectUntil.IsZero() {
		lines = append(lines, fmt.Sprintf("COLLECTION ACTIVE  |  ELAPSED T+%02d:%02d  |  MANUAL STOP", elapsedMin, elapsedSec))
	} else {
		...
		lines = append(lines, fmt.Sprintf("COLLECTION ACTIVE  |  ELAPSED T+%02d:%02d  |  T-%02d:%02d REMAINING", elapsedMin, elapsedSec, remainMin, remainSec))
	}
	lines = append(lines, fmt.Sprintf("%04d SAMPLES COLLECTED", len(app.CollectData)))
	...
	lines = append(lines, fmt.Sprintf("%04d UNIQUE PROCESSES", len(seen)))

	parts := make([]string, 0, 6)
	for _, r := range []string{"beacon", "pivot", "listener", "outbound"} {
		if n := roleCounts[r]; n > 0 {
			parts = append(parts, fmt.Sprintf("%s %04d", strings.ToUpper(r), n))
		}
	}
	if len(parts) > 0 {
	...
	lines = append(lines, "  PID       NAME                 TYPE")
	lines = append(lines, "  ────────  ───────────────────  ────────")
	added := 0
	for i := len(app.CollectData) - 1; i >= 0 && added < 6; i-- {
		...
		lines = append(lines, fmt.Sprintf("  %-8d  %-19s  %-8s", c.Proc.Pid, strings.ToUpper(common.ClipToWidth(info.name, 19)), strings.ToUpper(info.role)))
		added++
	}
...
func collectProgressLineStyle(line string) tcell.Style {
	...
	// New military format
	if strings.HasPrefix(trimmed, "COLLECTION ACTIVE") {
		return common.StyleCyan
	}
	if strings.HasPrefix(trimmed, "RECENT PROCESSES:") || strings.HasPrefix(trimmed, "  PID") || strings.HasPrefix(trimmed, "  ────") {
		return common.StyleMuted
	}
