...
func (m CalibrationModel) renderHeader() string {
	...
	return renderPanel(w, 3, "Calibration", "proxywatch", "", line)
}

// ── Setup panel ──────────────────────────────────────────────────────────────

func (m CalibrationModel) headerHeight() int { return 3 }
func (m CalibrationModel) setupHeight() int  { return 10 }
...
func (m CalibrationModel) renderSetup() string {
	...
	rows := []FormRow{
		...
		{Field: calibrateFieldModel, Label: "Model", Value: m.app.CalibrateModel},
		{Field: calibrateFieldProfile, Label: "Profile", Value: profileValue},
		{Field: calibrateFieldOutput, Label: "Output", Value: m.app.CalibrateOutput, Editable: true, CursorPos: m.app.CalibrateEditCursor},
		{Field: calibrateFieldDuration, Label: "Duration", Value: m.app.CalibrateDuration},
		{Field: calibrateFieldAction, Label: "Action", Value: actionIcon + " " + actionLabel},
		{Field: calibrateFieldApply, Label: "Apply", Value: "Apply selected profile"},
	}
...
func (m CalibrationModel) buildContent() string {
	...
	if app.CalibrateActive || app.CalibrateAnalyzing {
		...
		// A [*] line is "done" if any subsequent line exists (meaning we moved on).
		// Only the LAST [*] line is still pending.
		if app.CalibrateAnalyzing && len(progressLines) > 0 {
			plines := progressLines
			for i, line := range plines {
				trimmed := strings.TrimSpace(line)
				if strings.HasPrefix(trimmed, "[*]") {
					// Check if this is the last line — still pending.
					if i == len(plines)-1 {
						...
						// Past [*] lines have completed — show as done.
