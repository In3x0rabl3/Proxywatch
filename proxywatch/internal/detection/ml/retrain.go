...
func NewContinuousLearner(initialPredictor Predictor) *ContinuousLearner {
	...
	// Resume training records from the persisted buffer with a 24h
	// recency filter. The previous behavior wiped the buffer on every
	// startup, which forced headless deployments to spend ~45 min
	// collecting 200 records (the retrain trigger) per restart — visible
	// live on 2026-04-28 where buffer count crawled from 21 → 43 over
	// ten minutes after each restart, never reaching the threshold.
	// Recency filter handles the original concern (stale records from a
	// reconfigured box / updated vendor app) by dropping anything older
	// than a day. Clear() is preserved for the explicit operator-reset
	// path (training UI menu).
	const resumeMaxAge = 24 * time.Hour
	if err := cl.buffer.LoadFromDisk(resumeMaxAge); err == nil {
		shared.LogInfo("ml", "resumed training buffer: %d records (≤24h old)", cl.buffer.Len())
	}

	// Inject buffer-len reader into model.ShouldRetrain so the gate
	// can refuse retrain attempts when the buffer is below the
	// minimum-meaningful-batch threshold (500). Prevents the
	// thrashing pattern where retrains fired every 2 min on tiny
	// batches without enough new material to converge weights.
	model.SetBufferLenProvider(cl.buffer.Len)
...
func (cl *ContinuousLearner) attemptRetrain(reason string) {
	...
	// Minimum batch size for a meaningful retrain. Raised from 200 to
	// 500 (2026-05-02) because operator observed the model degrading
	// to DEGRADED state with 10 retrains in 3 hours on 99-231 record
	// batches — too small to converge against the listener-signal-
	// heavy class imbalance.
	if bufferSize < 500 {
		shared.SetCyclePhase(shared.CycleWaitingBuffer)
		shared.LogInfo("ml", "waiting for buffer: %d/500 records", bufferSize)
		return
	}
