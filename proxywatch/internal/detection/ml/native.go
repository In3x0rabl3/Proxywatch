...
func (m *nativeModel) PredictRole(fv features.FeatureVector) RolePrediction {
	...
	// Softmax to get probabilities. Apply temperature scaling so the raw
	// GBDT logits — which can run to ±50+ on deep trees with low samples-
	// per-leaf — don't saturate exp() into a degenerate ~1.0 distribution
	// across every prediction. Live telemetry on 2026-04-28 showed every
	// candidate (sshd, lsass, services, even a CS-style staged payload)
	// receiving TopProb≈1.0 — i.e. the model was usable for argmax-role
	// but its confidence channel was meaningless. Temperature divides
	// raw logits before exp, spreading the distribution without changing
	// argmax. T=3.0 was empirically chosen to bring observed top-prob
	// values down from 1.000 to ~0.6–0.9 on the demo box population
	// while keeping clear winners. Pure inference change — does not
	// require retraining or model re-export.
	probs := softmaxT(rawScores, 3.0)
...
// softmaxT is a temperature-scaled softmax. T=1.0 is the standard form;
// T>1 flattens the distribution (less peaked), T<1 sharpens it. Used at
// predict time to counter raw-logit saturation from deep GBDT ensembles.
// See PredictRole for the rationale.
func softmaxT(raw []float64, temperature float64) []float64 {
	if temperature <= 0 {
		temperature = 1.0
	}
	maxVal := raw[0]
