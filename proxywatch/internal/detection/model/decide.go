...
func trainingLabelToRole(label string) string {
	...
	case "outbound", "listener", "control-channel", "control-pivot":
		return label
	// Legacy label names — map to new roles.
	case "benign":
		return "outbound"
	case "malicious", "session", "control-session":
		return "control-channel"
	case "beacon", "control-beacon":
		return "control-channel"
	case "tunnel", "pivot":
...
func isSuspiciousRole(role string) bool {
	switch role {
	case "control-session", "control-beacon", "control-pivot",
		"control-channel", "control-tunnel", "tunnel", "smb-pipe":
		return true
	}
	return false
}
