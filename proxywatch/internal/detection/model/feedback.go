...
func RecordFeedback(entry FeedbackEntry) {
	...
	for _, sig := range entry.Signals {
		...
		ss.Total++
		// Operator action carries the same TP/FP semantics as the
		// experience-driven path in updateSignalStatsFromExperience:
		// kill confirms malicious, whitelist confirms benign. For
		// SUPPRESSION-class signals (outbound-known-vendor etc.) the
		// semantics invert because they predict "benign", so a
		// whitelist confirms them and a kill refutes them.
		isSuppress := shared.IsSuppressionSignal(sig)
		switch entry.Action {
		case "kill":
			if isSuppress {
				ss.FalsePositive++
