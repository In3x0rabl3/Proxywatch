...
type ProcessProfile struct {
	...
	TrainingLabel   string    `json:"training_label,omitempty"`   // "malicious", "benign", "tunnel", "beacon"
...
func Load() error {
	...
	if err != nil || len(data) == 0 {
		shared.LogInfo("model", "no saved model at %s (starting fresh)", modelPath)
		return nil
	}

	var loaded DetectionModel
	if err := json.Unmarshal(data, &loaded); err != nil {
		shared.LogWarn("model", "corrupt model JSON — starting fresh: %v", err)
		return nil
	}
	...
	if !verifyChecksum(&loaded, data) {
		...
		shared.LogInfo("model", "checksum mismatch (likely binary upgrade) — re-signing")
		loaded.Checksum = ""
	...
	if loaded.Version != modelVersion {
		shared.LogWarn("model", "version mismatch (got %d, want %d) — starting fresh", loaded.Version, modelVersion)
		return nil
	}
	...
	shared.LogInfo("model", "loaded %d profiles, %d feedback entries, %d signal stats",
...
// LookupSignalStat returns the signal-effectiveness stats for a given
// signal name, or nil if no stats have been recorded yet. Safe for
// concurrent reads. Used by the PCAP analyzer UI to display per-signal
// precision badges and by the pcap-mode rank.go scoring path to
// proportionally weight signal contribution by precision.
func LookupSignalStat(signal string) *SignalStat {
	mu.RLock()
	defer mu.RUnlock()
	if current == nil || current.SignalStats == nil {
		return nil
	}
	if ss, ok := current.SignalStats[signal]; ok && ss != nil {
		cp := *ss
		return &cp
	}
	return nil
...
// SetSignalStatForTest writes a SignalStat into the global model. Test-
// only — production code populates SignalStats via experience records.
// Used by detection tests to verify the training-signal filter.
func SetSignalStatForTest(signal string, ss *SignalStat) {
	mu.Lock()
	defer mu.Unlock()
	if current == nil {
		current = &DetectionModel{}
	...
	if ss == nil {
		delete(current.SignalStats, signal)
		return
	}
	cp := *ss
	current.SignalStats[signal] = &cp
}

// ResetSignalStatsForTest clears all signal stats — test helper for
// isolating test cases.
func ResetSignalStatsForTest() {
	mu.Lock()
	defer mu.Unlock()
	if current != nil {
		current.SignalStats = make(map[string]*SignalStat)
...
// The actual disk write happens in a background goroutine to avoid blocking
// the classifier and UI while holding the model lock.
func MaybeSave(now time.Time) {
	...
	// Snapshot the model data under lock, then release and write async.
	...
	// Write to disk outside the lock — no blocking.
	go func() {
		dir := filepath.Dir(path)
		if dir != "" && dir != "." {
			_ = os.MkdirAll(dir, dirMode)
		}
		_ = os.WriteFile(path, dataWithChecksum, fileMode)
		_ = keystore.VaultWrite(vaultName, dataWithChecksum, path)
	}()
}
...
// patterns, signal stats, quality metrics) while preserving egress paths
// discovered by contour. The model reverts to a clean state and maturity
// counters are zeroed.
func ResetToBaseline() {
	...
	current.Processes = make(map[string]*ProcessProfile)
	current.Feedback = nil
	current.TrainingPatterns = nil
	current.SignalStats = make(map[string]*SignalStat)
	current.Quality = QualityMetrics{}
	current.SelfValidations = make(map[string]SelfValidation)
	current.HostOverlays = make(map[string]*HostOverlay)
	current.CalibrationRuns = 0
	current.CalibrationSamples = 0
	current.CalibrationContamination = 0
	current.Maturity = ModelMaturity{}
	current.BaselineState = ""
	current.BaselineVersion = ""
	current.BaselineType = ""
	current.BaselineReadyAt = time.Time{}
	current.BaselineAccuracy = 0
	current.BaselineDataSize = 0
	current.UpdatedAt = time.Now().UTC()
	dirty = true
	ResetAllMaturityCounters()

	// Increment model generation so all processes re-enter analyzing state.
	shared.ModelGeneration++

	// Clear all classifier state maps (roles, statuses, connection tracking).
	// This ensures processes don't retain old Pivot/Beacon/Tunneling states
	// after RTB.
	shared.ClearClassifierState()

	// Immediately reset all behavior observations so the classifier and
	// CandidateState see the new generation without waiting for
	// GetOrCreateProcessBehavior to be called via the scoring path.
	for _, b := range shared.ProcessBehaviorByKey {
		b.Observations = 0
		b.SuspiciousObservations = 0
		b.StrongObservations = 0
		b.ActiveObservations = 0
		b.Generation = shared.ModelGeneration
	}
}
...
// DeleteProfilesForHost wipes every per-process profile whose key
// belongs to the given host scope. The pcap analyzer uses this to
// guarantee deterministic re-analysis: without it, the model
// accumulates committed-role / dominant-role / experience-count
// state from prior runs (visible as "model: smb-pipe (96% of 130
// observations)" reasons drifting between identical analyses on the
// same pcap file). The cleanup is host-scoped so live-capture
// profiles for real hosts are never affected — only the synthetic
// "pcap-replay" host's profiles get removed.
func DeleteProfilesForHost(host string) {
	if host == "" || current == nil {
		return
	}
	mu.Lock()
	...
	if current.Processes != nil {
		for key := range current.Processes {
			if hostFromKey(key) == host {
				delete(current.Processes, key)
			}
		}
	}
	if current.HostOverlays != nil {
		delete(current.HostOverlays, host)
	}
}
...
func applyDecay(m *DetectionModel) {
	...
	for key, profile := range m.Processes {
		...
		// Reset premature benign verdicts: profiles auto-labeled benign with
		// fewer than 500 observations likely didn't have enough data for
		// beacon/C2 patterns to emerge. Reset to unknown so detection can
		// re-evaluate with more evidence.
		if profile.CalibrationVerdict == "benign" &&
			profile.UserVerdict == "" &&
			profile.CalibrationRuns < 3 &&
			profile.ExperienceObservations < 500 {
			profile.CalibrationVerdict = "unknown"
			profile.OverallConfidence = computeConfidence(profile)
		}
	}
...
func computeConfidence(p *ProcessProfile) float64 {
	...
	// Experience-based role stability — only contributes after substantial
	// observation to avoid racing to benign/malicious prematurely.
	if p.ExperienceObservations >= 200 && p.RoleStability > 0.9 {
		if p.DominantRole == "outbound" || p.DominantRole == "listen" {
			c += 0.1
		} else if isSuspiciousRole(p.DominantRole) {
			c -= 0.1
		}
	}
