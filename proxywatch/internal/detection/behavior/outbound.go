...
func EmitOutboundSignals(c *shared.Candidate, addSignal func(string), ctx SignalContext, cs CommonState) {
	...
	// Kept active on pcap clusters even though it sometimes vetoes real
	// beacons (d10_1h, merlin_http2 corpus runs 2026-05-04). Removing it
	// caused a clear regression on test1.pcap: 172.16.1.139 → api.anthropic.com
	// (a legitimate API client) carries tls-rare-signature + shape and
	// would promote without this veto. Without the byte/cadence
	// volume to disambiguate API clients from beacons, the suppressor
	// is the safer default — accept the FN on d10/merlin until the
	// per-cluster shape stamper can use packet-level IATs (which would
	// reveal the regular beat structure that an API client lacks).
	...
	// SKIPPED on pcap per-/16 cluster candidates — synthetic cluster PIDs
	// have no real per-process behavior history; the "baseline" learned
	// against a destination-cluster fingerprint isn't process-identity
	// evidence. Operator-confirmed 2026-05-04 on mythic_24hr.pcap: the
	// Mythic /16 cluster fired TS+DS+rare-sig+day-rhythm yet got Path D
	// vetoed by this signal because the 24h capture accumulated enough
	// "observations" against the cluster's synthetic key to pass the
	// stability gate. Live-mode + rollup candidates retain the signal.
	if behaviorStable && !shared.IsPcapPerDestCluster(c) {
