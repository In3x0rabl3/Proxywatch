...
doneInboundExternal:
	...
	//
	// REMOVED 2026-05-02 from emission. Operator dashboard showed this
	// signal at 0% precision (TP=2, FP=178,211) — Windows hosts have
	// 50+ legitimate listeners on "uncommon" ports (RPC dynamic ports
	// in 49152-65535, named-pipe-over-TCP services, sshd alternate
	// configs, etc.). The signal contributed ~178K false positives to
	// the model's training data while producing only 2 true positives.
	// Disabling emission removes the noise without changing detection:
	// real implants are caught by the beacon-* / pivot-* / session-*
	// signals that have meaningful precision.
	//
	// Kept the gating block (commented) so future tightening (e.g.
	// require uncommon-port AND no-children AND no inbound for ≥X
	// minutes) can be re-enabled if a use case appears.
	//
