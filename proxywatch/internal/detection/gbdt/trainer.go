...
// HyperParamGrid defines ranges for grid search.
type HyperParamGrid struct {
	NEstimators    []int
	MaxDepth       []int
...
// DefaultHyperParamGrid returns a reasonable search grid.
func DefaultHyperParamGrid() HyperParamGrid {
	return HyperParamGrid{
		NEstimators:    []int{100, 200, 300},
		MaxDepth:       []int{4, 6, 8},
		LearningRate:   []float64{0.05, 0.1, 0.15},
		MinSamplesLeaf: []int{5, 10, 20},
	}
}

// SmallHyperParamGrid returns a minimal grid for quick tuning.
func SmallHyperParamGrid() HyperParamGrid {
	return HyperParamGrid{
		NEstimators:    []int{100, 200},
		MaxDepth:       []int{4, 6},
		LearningRate:   []float64{0.1},
		MinSamplesLeaf: []int{10},
	}
}

// TuneResult holds the outcome of a single hyperparameter configuration.
type TuneResult struct {
	Params  HyperParams
	Metrics *EvalMetrics
	Score   float64 // primary optimization metric (MacroF1)
}

// GridSearchCV performs grid search with cross-validation to find best hyperparameters.
// Returns the best params and all results sorted by score descending.
func GridSearchCV(ctx context.Context, ds *Dataset, grid HyperParamGrid, cvFolds int) (HyperParams, []TuneResult, error) {
	if cvFolds < 2 {
		cvFolds = 3
	}
	...
	bestScore := -1.0
	bestParams := DefaultHyperParams()

	// Generate all combinations
	for _, nEst := range grid.NEstimators {
		for _, depth := range grid.MaxDepth {
			for _, lr := range grid.LearningRate {
				for _, minLeaf := range grid.MinSamplesLeaf {
					if ctx.Err() != nil {
						return bestParams, results, ctx.Err()
					}
					...
					metrics, err := TimeSeriesCV(ctx, ds, params, cvFolds)
					if err != nil {
					...
					result := TuneResult{
						Params:  params,
						Metrics: metrics,
						Score:   metrics.MacroF1,
					}
					results = append(results, result)

					if metrics.MacroF1 > bestScore {
						bestScore = metrics.MacroF1
						bestParams = params
					}
				}
			}
	...
	return bestParams, results, nil
}

// QuickTune runs a fast hyperparameter search with a small grid.
// Use this when time is limited.
func QuickTune(ctx context.Context, ds *Dataset) (HyperParams, error) {
	best, _, err := GridSearchCV(ctx, ds, SmallHyperParamGrid(), 3)
	return best, err
}
