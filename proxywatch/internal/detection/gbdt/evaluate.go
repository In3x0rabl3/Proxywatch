...
type EvalMetrics struct {
	...
	ControlRecall float64                 `json:"control_recall"` // min recall across control-* classes with support
...
func CheckQualityGate(metrics *EvalMetrics, gate QualityGate) (pass bool, failures []string) {
	...
	if metrics.ControlRecall < gate.MinControlRecall {
		failures = append(failures, fmt.Sprintf("control_recall %.3f < %.3f", metrics.ControlRecall, gate.MinControlRecall))
		pass = false
	}
...
func metricsFromConfusion(confusion [][]int, nc, n, correct int) *EvalMetrics {
	...
	for c := 0; c < nc; c++ {
		...
		// Track control-class recall (only for classes with support).
	...
	if controlClassesSeen > 0 {
		...
		// No control-class samples to evaluate — set to 1.0 so gate doesn't block.
		m.ControlRecall = 1.0
	}
