...
func GetUDPTable() ([]shared.UDPListenerInfo, error) {
	l4, err := getUDPTableForFamily(AF_INET)
	if err != nil {
		return nil, err
	}
	l6, err := getUDPTableForFamily(AF_INET6)
	if err != nil {
		return l4, nil
	}
...
func getUDPTableForFamily(family uint32) ([]shared.UDPListenerInfo, error) {
	...
	r0, _, _ := ProcGetExtendedUdp.Call(
		0,
		uintptr(unsafe.Pointer(&size)),
		0,
		uintptr(family),
	...
	r0, _, e1 := ProcGetExtendedUdp.Call(
		uintptr(unsafe.Pointer(&buf[0])),
		uintptr(unsafe.Pointer(&size)),
		0,
		uintptr(family),
	...
	if family == AF_INET {
		rowSize := int(unsafe.Sizeof(MIBUDPROwnerPID{}))
		maxRows := 0
		if used > rowStart && rowSize > 0 {
			maxRows = (used - rowStart) / rowSize
		}
		...
		for i := 0; i < rows; i++ {
			...
			row := (*MIBUDPROwnerPID)(unsafe.Pointer(&buf[off]))
			out = append(out, parseUDPv4(row))
		}
	} else {
		rowSize := int(unsafe.Sizeof(MIBUDP6OwnerPID{}))
		maxRows := 0
		if used > rowStart && rowSize > 0 {
			maxRows = (used - rowStart) / rowSize
		}
		...
		for i := 0; i < rows; i++ {
			...
			row := (*MIBUDP6OwnerPID)(unsafe.Pointer(&buf[off]))
...
func parseUDPv4(r *MIBUDPROwnerPID) shared.UDPListenerInfo {
	if r == nil {
...
func parseUDPv6(r *MIBUDP6OwnerPID) shared.UDPListenerInfo {
	if r == nil {
		return shared.UDPListenerInfo{}
	}
	lip := net.IP(r.LocalAddr[:]).String()
...
func GetRawSocketPIDs() map[int]bool {
	...
	for _, family := range []uint32{AF_INET, AF_INET6} {
...
func EnumerateNamedPipes() []shared.NamedPipeInfo {
	...
	for attempts := 0; attempts < 3; attempts++ {
		...
		for i := uint32(0); i < count && lookups < maxLookups; i++ {
			...
			entry := (*SystemHandleEntry)(unsafe.Pointer(&buf[entryOffset]))
			...
			nameStatus, _, _ := ProcNtQueryObject.Call(
