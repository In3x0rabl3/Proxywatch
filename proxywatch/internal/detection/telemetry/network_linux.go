...
func Collect() (*shared.Snapshot, error) {
	...
	pipes := enumerateLinuxNamedPipes(procs)
...
// enumerateLinuxNamedPipes scans /proc/[pid]/fd for Unix domain sockets
// and named FIFOs. These are the Linux equivalent of Windows named pipes
// and are used by SMB/Samba for lateral movement.
func enumerateLinuxNamedPipes(procs map[int]*shared.ProcessInfo) []shared.NamedPipeInfo {
	...
	for pid := range procs {
		...
		for _, entry := range entries {
			...
			// Unix domain sockets show as "socket:[inode]".
			// Named FIFOs show as actual paths.
			if strings.HasPrefix(link, "socket:[") {
				continue // Skip regular sockets — too noisy.
			}
			// Check if link target is a FIFO (named pipe).
