...
func applyRoleWarmupGates(
	...
	case "control-channel":
		minSecs := max(60, int(shared.SessionMinLabelDuration.Seconds())-warmupReduction)
		if c.Role == "control-channel" {
			minSecs = max(120, int(shared.BeaconMinLabelDuration.Seconds())-warmupReduction)
		}
		age := sessionAge
		if c.Role == "control-channel" {
			age = beaconAgeSecs
...
func canPromoteBeaconRole(
	...
	// inherently create control-like connections on each callback.
