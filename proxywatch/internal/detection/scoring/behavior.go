...
func ApplyBehaviorAwareAdjustments(
	...
	if benignClient &&
		...
		c.Reasons = append(c.Reasons, "Weak external-only beacon shape downgraded pending stronger corroboration")
	}
}
...
func ApplySignalFusionAdjustments(
	...
	suspiciousFusion := countSignals(
		"beacon",
		"beacon-attempt-repeated",
		"beacon-target-stable",
	...
	if c.Role == "beacon" &&
		...
		c.Reasons = append(c.Reasons, "Signal fusion downgraded weak benign-looking beacon pattern")
		return
	}

	// Promote strong multi-trigger control behavior that still sits in outbound-only.
	//
	// Pcap-mode gate: signal-fusion ≥3 is satisfied by packet-only beacon /
	// session signals on synthetic candidates that have no process metadata
	// to suppress them (vendor-signed-trusted, pkg-owned, etc. never fire
	// in pcap). Cloud-SaaS and admin-tool flows trip 3+ shape signals
	// routinely and get promoted to beacon here. Defer pcap-mode
	// promotion to ApplyPcapModeRoleGuard / demoteUnderEvidencedSyntheticPivots
	// which require a packet-decisive signal or a hard-distinguisher.
	if c.Role == "outbound" &&
		...
		if holdBenignExternalPromotion {
			...
			c.Reasons = append(c.Reasons, "Signal fusion promoted multi-trigger control behavior to beacon")
		}
	}
}
