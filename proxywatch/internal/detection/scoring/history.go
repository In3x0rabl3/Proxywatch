...
func UpdateConnHistory(pid int, conns []shared.ConnectionInfo, now time.Time) {
	...
	for _, cn := range conns {
		...
		// Cluster-level destination first-seen — keyed on
		// (LocalIP, /24-prefix, RemotePort), survives restarts and
		// process exits. First-seen scoring reads this.
		if cn.RemoteAddress != "" && cn.RemotePort > 0 &&
			!shared.IsLoopbackIP(cn.RemoteAddress) && !shared.IsWildcardIP(cn.RemoteAddress) {
			shared.RecordDestFirstSeen(cn.LocalAddress, cn.RemoteAddress, cn.RemotePort, now)
		}
	}
...
func FindPersistentControl(pid int, conns []shared.ConnectionInfo, listenerPorts map[int]struct{}, now time.Time) (*shared.ConnectionInfo, int) {
	...
	for _, cn := range conns {
		...
		// Skip connections accepted on listener ports — these are inbound
		// clients, not outbound control channels. An SSH server accepting
		// a connection on port 22 is normal, not a reverse shell.
		if _, isListener := listenerPorts[cn.LocalPort]; isListener {
			continue
		}
...
func PurgeHistory(now time.Time) {
	...
	// Expire stale TunnelingSeen stamps. Without this, a single
	// ActiveProxying=true cycle pins a process as "tunneling" until
	// HistoryTTL (2h) elapses, even though no relay traffic is flowing.
	// 60s mirrors the pivotLingerWindow — a tunnel that hasn't seen
	// any activity in 60s is no longer "currently tunneling".
	const tunnelingSeenTTL = 60 * time.Second
	for pid, last := range shared.TunnelingSeen {
		if now.Sub(last) > tunnelingSeenTTL {
