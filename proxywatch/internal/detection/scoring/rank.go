...
// pcapSignalPrecisionFloor and the precision-filter logic was
// REVISITED 2026-05-03 after a regression — the initial 0.30 floor
// dropped LOAD-BEARING decisive signals like child-tunnel-relay
// (8% precision) and beacon-reconnecting-unknown-vendor (11%) whose
// low TP/FP ratio is structural (they fire on many outbound clusters
// that ultimately classify as legit, not because the signal itself
// is unreliable when corroborated). Those low-precision-but-
// load-bearing signals are exactly the ones role-guard relies on.
//
// New approach: only filter signals that appear on
// pcapNeverPromoteSignals — a curated allowlist of signals known to
// produce only noise when isolated (no role-guard path uses them as
// decisive evidence). Everything else passes through emission
// regardless of measured precision; the role guard's combination
// rules sort decisive vs non-decisive downstream.
//
// Signals on this list are suppressed at addSignal time in pcap
// mode only. Live mode is unchanged.
var pcapNeverPromoteSignals = map[string]bool{
	// listener-uncommon-port had TP=2 / FP=178,211 in the operator's
	// 2026-05-02 dashboard — fires on every Windows RPC dynamic port,
	// named-pipe-over-TCP service, sshd alternate config, etc.
	// Already removed from emission in behavior/listener.go (commented
	// out); kept here as belt-and-braces in case a future change
	// re-enables it.
	"listener-uncommon-port": true,
}

// pcapSignalDropByPrecision reports whether a signal should be
// suppressed in pcap-mode emission. Currently only filters signals
// in the pcapNeverPromoteSignals allowlist; the broader precision
// filter was rolled back after it killed load-bearing decisive
// signals. The full per-signal precision data is still surfaced in
// the PCAP detail panel via Phase 1.1's badge so operators can
// triage by reliability.
func pcapSignalDropByPrecision(signal string) bool {
	return pcapNeverPromoteSignals[signal]
}
...
func ScoreCandidate(c *shared.Candidate, now time.Time) {
	...
	addSignal := func(s string) {
		...
		// Pcap-mode precision filter: drop signals whose measured TP/FP
		// precision is below pcapSignalPrecisionFloor. Live mode is
		// unchanged — the filter only applies when IsPcapMode(c). This
		// stops the "listener-uncommon-port at 0% precision" failure
		// mode (the signal had TP=2 / FP=178,211 in production) without
		// removing it from emission entirely (live mode keeps every
		// signal so its operator-feedback-driven precision keeps
		// updating).
		if shared.IsPcapMode(c) && pcapSignalDropByPrecision(s) {
			return
		}
	...
	if now.IsZero() {
		// shared.PcapNow returns the pcap-anchored time when pcap ingest
		// is active, otherwise time.Now(). In pcap mode this gives
		// scoring deterministic "now" across the per-window classify
		// loop AND across re-analyses of the same pcap.
		now = shared.PcapNow()
	}

	// SSH baseline learning: track SSH patterns per user to detect lateral movement.
	// Adds signals like "ssh-first-time-destination" for new destinations.
	sshSignals := bhv.GetSSHBaseline().RecordSSHConnection(c)
	for _, sig := range sshSignals {
		addSignal(sig)
	}
	...
	// outInternalEstablished counts internal-target connections in
	// ESTABLISHED state ONLY — used by every "is this process actively
	// relaying right now" gate (reverseProxyNow, isTunnelShape, the
	// late-pass active-relay heuristic). Without this gate, a single
	// CLOSE_WAIT remnant pinned cheerful_glove.exe to State="tunneling"
	// indefinitely after a finished SOCKS scan: the CLOSE_WAIT counted
	// as "internal", reverseProxyNow stayed true, ActiveProxying stayed
	// true, and the 10-minute SuspicionProxy window kept refreshing
	// every cycle. The all-states `outInternal` is preserved for paths
	// that legitimately want "any internal activity" (signal counters,
	// historical scoring); only the freshness-relevant gates use the
	// ESTABLISHED-only count.
	outInternalEstablished := 0
	for _, cn := range c.Conns {
		if cn.RemoteAddress == "" || shared.IsLoopbackIP(cn.RemoteAddress) || !shared.IsInternalIP(cn.RemoteAddress) {
			continue
		}
	...
	if controlConn != nil && !hasListener {
		...
		// Count ESTABLISHED-only outbound (excluding the control conn)
		// for the reverseProxyNow trigger. The all-states variant
		// counted CLOSE_WAIT remnants and pinned ActiveProxying=true
		// long after a SOCKS scan finished — see the
		// outInternalEstablished comment at the top of ScoreCandidate.
		proxyOutEstablished := 0
		for _, cn := range c.Conns {
			...
			ck := ConnKeyFromConn(scopedPID, cn)
			if ck == controlKey {
				continue
			}
			proxyOutEstablished++
		}
		if proxyOutEstablished > 0 && reverseTunnelEligible {
			reverseProxyNow = true
		}
	}

	// Internal-target reverse-proxy trigger: must have at least one
	// ESTABLISHED internal connection (a closing CLOSE_WAIT remnant
	// is no longer a "currently relaying" signal).
	if !reverseProxyNow && !hasListener && outInternalEstablished > 0 && reverseTunnelEligible {
		reverseProxyNow = true
	}
	...
	for _, sig := range signals {
		...
		case "beacon-tight-cadence":
			// Short interval + low jitter is strong behavioral evidence
			beaconCorroboration += 2
		case "outbound-asn-org-aligned":
			hasASNAligned = true
		}
	}
	...
	// ASN-aligned destinations (connecting to vendor's network) strongly
	// indicate legitimate telemetry, not C2. Reduce corroboration.
	if hasASNAligned && promoIsKnownVendor {
		beaconCorroboration -= 3
	}
	...
	// below so it can claim the pivot role even when the process
	...
	if reverseProxyNow || (suspiciousRecent && hist.SuspicionKind == shared.SuspicionProxy && !singleControlNoProxy) {
		// Don't downgrade pivot (more specific) to beacon.
		if c.Role != "pivot" {
			c.Role = "beacon"
		}
		c.ActiveProxying = true
	...
	// Promote persistent beacon shapes to session even when reverse-control
	...
	// SSH client connections to internal infrastructure are normal admin activity.
	// Do NOT promote these to beacon/session regardless of connection shape.
	if shared.IsSSHClientToInternal(c) {
		c.Reasons = append(c.Reasons, "SSH client to internal infrastructure (benign admin activity)")
	} else if c.Role == "outbound" &&
	...
	if c.DelegatedStrong &&
		...
		c.Role = "beacon"
		c.ActiveProxying = false
		c.Reasons = append(c.Reasons, "Delegated beacon shape is consistent with a reverse session")
		if c.Score < 45 {
			c.Score = 45
		}
	}
	...
	if c.Role == "outbound" &&
		...
		if pureCallbackShape && !promoIsKnownVendor && shared.ShortLivedBurstHits[scopedPID] >= 3 {
			...
			if offlineBenign {
				c.Signals = shared.AppendUniqueSignal(c.Signals, "benign-offline-service-shape")
			} else {
	...
	if CanPromoteBeaconRole(
		...
		// SSH client to internal infrastructure is always benign admin activity.
		if !beaconBlocked && beaconEligible && (beaconConfirmed || beaconRecent || provisionalBeacon) && beaconCorroboration >= 3 && !shared.IsSSHClientToInternal(c) {
			c.Role = "beacon"
			c.ActiveProxying = false
	...
	// Uses the ESTABLISHED-only outInternalEstablished computed at the
	// top of ScoreCandidate — see the rationale comment there.
	isTunnelShape := outExternal > 0 && outInternalEstablished >= 1 && len(distinctTargetPorts) >= 2
	// Also match with higher internal count regardless of port diversity.
	// Uses all-states count here because high-volume scan bursts often
	// have many connections in flight (mix of ESTABLISHED + CLOSE_WAIT).
	if outExternal > 0 && outInternal >= 3 {
		isTunnelShape = true
	}
	// Traditional tunnel: listener + external + internal.
	if (c.Role == "tunnel" || c.Role == "beacon" || forwardTunnelLikely || reverseProxyNow) &&
		hasListener && outExternal > 0 {
		isTunnelShape = true
	}

	// SMB pipe-like behavior should not remain plain outbound/listener.
	// But don't override tunnel/beacon — SOCKS tunnels have internal
	...
	// Re-evaluate tunnel shape after role changes above. Same ESTABLISHED-only
	// gate on the low-internal-count path — see the cheerful_glove rationale
	// above the first isTunnelShape definition.
	isTunnelShape = (outExternal > 0 && outInternalEstablished >= 1 && len(distinctTargetPorts) >= 2) ||
		(outExternal > 0 && outInternal >= 3) ||
		((c.Role == "tunnel" || c.Role == "beacon" || forwardTunnelLikely || reverseProxyNow) &&
			hasListener && outExternal > 0)
	...
	// to beacon so the user sees "tunneling" in the state column
	...
	if !c.StrongEvidence && c.Role == "beacon" && controlConn != nil {
		...
		if outInternal > 0 || internalLateral || c.DelegatedStrong || (controlSecs >= 180 && !suppressStrongExternalSession) {
			...
			c.Reasons = append(c.Reasons, "Sustained session evidence exceeded strong threshold")
	...
	//
	// Gate: only promotes from beacon / listen / listener so we
	...
	// ActiveProxying actually firing.
	{
		...
		// as pivot. The (tunnelingRecent && pivotRecent) combination
		...
		if (c.Role == "beacon" || c.Role == "listen" || c.Role == "listener" || c.Role == "outbound") &&
			tunnelingRecent &&
			pivotRecent {
			c.Role = "pivot"
	...
	// SSH client to internal infrastructure: demote ANY malicious role to outbound.
	// Plain SSH sessions (no tunnel flags) to internal IPs are normal admin activity.
	// SSH with tunnel flags is handled separately and can still be flagged.
	if shared.IsSSHClientToInternal(c) && IsMaliciousRole(c.Role) {
		c.Role = "outbound"
		c.ActiveProxying = false
		c.Reasons = append(c.Reasons, "SSH client to internal infrastructure (plain session, no tunnel flags)")
	...
	// hist.LastRole / hist.LastRoleChange are updated by classifier.go
	// AFTER the per-candidate post-rank passes (DemoteShapeOnlyControl,
	// ApplyVendorFPShape, ApplyCaptureToolSuppression, ML prediction,
	// signal override) commit the FINAL role for the cycle. Writing
	// them here would record rank.go's pre-demote role and then the
	// classifier's streak-stability comparison (`hist.LastRole == c.Role`)
	// would always mismatch on demoted candidates — the streak would
	// reset to 1 on every cycle and `analyzing` state would never
	// exit. Streak updates belong at the final role-commit point only.

	// Determine beacon subtype from signals.
	if c.Role == "beacon" {
		hasBeaconSig := false
		hasSessionSig := false
		for _, sig := range signals {
			...
			if sig == "beacon" || sig == "persistent-control" || sig == "reverse-control" || sig == "strong-session" {
				hasSessionSig = true
			}
		}
	...
	if !c.ActiveProxying {
		...
		//
		// FRESHNESS GATE (added 2026-05-03): in addition to the topology
		// shape, require some current-cycle activity. Without this, a
		// cheerful_glove-style pivot sticks to ActiveProxying=true
		// forever — its 8 long-held ESTABLISHED externals + 1 CLOSE_WAIT
		// internal trip outTotal>=4 + distinctPorts>=2 every cycle, even
		// when no relay traffic is actually flowing (0 B/s, no fresh
		// internal connections). Tunneling means "carrying relay traffic
		// right now," not "has the topology of a tunnel."
		//
		// Activity = IO bps above the keepalive floor (≥512 B/s) OR at
		// least one ESTABLISHED internal connection (real relay needs an
		// inside endpoint, not a closing remnant).
		...
		if hasActiveRelay {
			ioActive := c.Proc != nil && (c.Proc.IOReadBps >= 512 || c.Proc.IOWriteBps >= 512)
			internalEstablished := outInternalEstablished > 0
			if ioActive || internalEstablished {
				c.ActiveProxying = true
			}
		}
	}
