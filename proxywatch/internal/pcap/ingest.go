...
// Ingest opens a pcap/pcapng file, walks every packet, slices flows
// into 1s windows, replays each window through detection.Classify, and
// reports the union of detector activity. Cancellation via ctx aborts
// immediately.
//
// The function holds shared.ClassifyMu via Classify, so live capture
// cycles are paused for the duration of replay (one Classify at a time).
// Synthetic-PID state in shared package globals is wiped before return.
func Ingest(ctx context.Context, path string) IngestResult {
	// Route .log files (Zeek logs) to the Zeek parser
	if isZeekPath(path) {
		return IngestZeek(ctx, path)
	}
	return IngestWithProgress(ctx, path, nil)
}

// isZeekPath returns true if the path looks like a Zeek log file or directory.
func isZeekPath(path string) bool {
	ext := strings.ToLower(filepath.Ext(path))
	if ext == ".log" || ext == ".gz" {
		// Check if parent dir has conn.log (Zeek logs always have conn.log)
		base := strings.ToLower(filepath.Base(path))
		if strings.HasPrefix(base, "conn.") || strings.HasPrefix(base, "dns.") ||
			strings.HasPrefix(base, "ssl.") || strings.HasPrefix(base, "http.") ||
			strings.HasPrefix(base, "x509.") {
			return true
		}
		// Check if it's a directory with conn.log
...
// IngestWithProgress is the channel-aware variant. progressCh receives
// non-blocking sends; drops are fine (UI re-polls). Pass nil to skip.
func IngestWithProgress(ctx context.Context, path string, progressCh chan<- IngestProgress) IngestResult {
	// Disable DNS PTR / forward refresh goroutines while pcap is
	// running. Pcap-replay against a 50+ MB capture floods these with
	// hundreds of unique remote IPs simultaneously and crashes the
	// Windows DNS API. See shared.PcapModeActive doc for full
	// rationale. Cleared on return.
	...
	// Reorient mid-stream flows where the SYN was never observed.
	// Without a SYN, the InitIP/RespIP fields are set by whatever
	// direction's first packet arrived in the capture window — which
	// for any flow that started before the capture began will be the
	// SERVER's response (since the client is patiently waiting for
	// data). That mis-attribution turns a plain outbound HTTPS
	// download into a phantom listener candidate. Flip whenever the
	// well-known-port heuristic disagrees with the packet-order guess
	// (e.g. 443 > 46154 → 443 is the responder).
	reorientSynlessFlows(flows)

	// Time-shift every flow so the first packet lands at a wallclock
	// equal to time.Now(). The orchestrator compares snapshot timestamps
	// against shared.ProxywatchStartedAt and other live wallclock state
	// (StartupGracePeriod, ConnMissingGrace, suspicion windows). Without
	// a shift, pcap timestamps from days/years ago break those gates and
	// most signals never fire. The relative deltas between snapshots are
	// preserved, so timing-based detectors (beacon-interval, syn-cycle,
	// session-persistence) see the same rhythm as live capture.
	...
	// Anchor scoring's wall-clock view to the post-shift end time so
	// every time.Now() inside the classifier (now routed through
	// shared.PcapNow) returns the same value across the entire ingest
	// AND across re-analyses of the same pcap. Without this anchor,
	// time.Now() drifts during the per-window classify loop and across
	// runs by the elapsed wall-clock seconds, flipping role assignments
	// on weak-evidence candidates whose linger / freshness gates compare
	// "now" against shifted flow timestamps.
	shared.PcapClockNanos.Store(shiftedEnd.UnixNano())
	defer shared.PcapClockNanos.Store(0)
	// unshift converts a now-relative timestamp (post-shift, what the
	// orchestrator sees) back to the original pcap-recorded time. Used
	// when populating operator-facing fields like FirstPacketByPID,
	// PeakWindowByPID, FlowSummary timestamps — those need to be in
	// the same time frame as result.PcapStart so pcap-relative
	// offsets like "T+02:34" make sense to the analyst.
	...
	// Prime ProxywatchStartedAt to a moment well before our shifted
	// replay so the rank.go pre-existing-tunnel detection's
	// StartupGracePeriod gate is satisfied. Restored after replay.
	...
	// pidByIP keeps per-IP stats accounting (PacketsByPID, BytesByPID,
	// etc. — operator-facing per-host columns). The classifier itself
	// runs on the per-endpoint attribution below so each listener port
	// + the catch-all outbound aggregator classify independently.
	...
	// Reset orchestrator state for our synthetic PIDs / behavior keys
	// BEFORE the replay starts. Without this, leftover state from a
	// prior pcap run (or a crashed run that skipped cleanup) seeds the
	// behavioral baselines and the second analysis of the same pcap
	// produces a different verdict than the first. Cleanup runs on
	// return as well so live state is never polluted by replay.
	resetSyntheticPIDState(result.SyntheticPIDs)
	defer cleanupSyntheticPIDState(result.SyntheticPIDs)

	// Build a sorted view of flows so snapshot construction is
	// deterministic per run — Go map iteration order is randomized,
	// and the order we emit Listeners / Connections into a snapshot
	// affects the order the orchestrator sees them, which can in turn
	// affect signal-firing edge cases.
	sortedFlows := sortFlowsForReplay(flows)

	// Per-host totals for the packet view's columns. Each flow
	// contributes to whichever local-side synthetic PID it touches —
	// init or resp. A flow between two local IPs (e.g. internal-to-
	// internal lateral movement) increments both.
	//
	// Direction attribution: from the LOCAL side's perspective, "out"
	// is bytes the local host SENT, "in" is bytes it RECEIVED.
	//   - local is initiator: out = init→resp, in = resp→init
	//   - local is responder:  out = resp→init, in = init→resp
	...
	// Build first/last seen first, then derive observation duration.
	firstSeen := make(map[int]time.Time, len(pidByIP))
	lastSeen := make(map[int]time.Time, len(pidByIP))
	bumpRange := func(dst map[int]time.Time, pid int, t time.Time, takeMin bool) {
		existing, ok := dst[pid]
		if !ok || (takeMin && t.Before(existing)) || (!takeMin && t.After(existing)) {
			dst[pid] = t
		}
	}
	bumpIP := func(pid int, first, last time.Time) {
		bumpRange(firstSeen, pid, first, true)
		bumpRange(lastSeen, pid, last, false)
	}
	// Per-endpoint timing + flow-level decision data, keyed by the
	// attribution-allocated PIDs used for the actual candidates. The
	// TUI looks up by candidate PID, so this is the map analysts see.
	...
	addEndpointBytes := func(pid int, st *flowState, summary FlowSummary) {
		// unshift back to the original pcap-recorded time so pcap-
		// relative offsets in the TUI line up with PcapStart (which
		// is also original). Without this, T+offset reads "T+1h06m"
		// on a 1m21s pcap because the shift puts firstPacket in
		// wallclock-now territory.
		...
		// Hour-of-day distribution — only useful when the pcap spans
		// hours; the renderer suppresses the heatmap section for
		// short pcaps but populating it is cheap so we always do.
		if pcapSpanHours >= 1 {
	...
	if result.FlowsMeta == nil {
		result.FlowsMeta = make(map[FlowID]FlowMeta, len(sortedFlows))
	}
	for _, st := range sortedFlows {
		...
		// Per-flow metadata for view-time projections (JA3 / SNI / ASN
		// / session / behavior groupings in the TUI). Captures what
		// flowState already extracted during packet processing.
		fid := FlowID{
			LocalIP:    st.key.InitIP,
			LocalPort:  st.key.InitPort,
			RemoteIP:   st.key.RespIP,
		...
		result.FlowsMeta[fid] = FlowMeta{
			...
			HTTPURI:   st.httpURI,
			BytesSum:  flowBytes,
			FirstSeen: unshift(st.firstPacket),
			LastSeen:  unshift(st.lastPacket),
		}
		if pid, ok := pidByIP[st.key.InitIP]; ok {
			...
			bumpIP(pid, st.firstPacket, st.lastPacket)
		}
		if pid, ok := pidByIP[st.key.RespIP]; ok && st.key.InitIP != st.key.RespIP {
			result.PacketsByPID[pid] += flowPackets
			result.FlowsByPID[pid]++
			result.BytesByPID[pid] += flowBytes
			result.BytesOutByPID[pid] += st.bytesRespToInit
			result.BytesInByPID[pid] += st.bytesInitToResp
			bumpIP(pid, st.firstPacket, st.lastPacket)
		}
		// Per-endpoint accounting (drives the TUI candidate views):
		// outbound flow → outboundPIDFor(local, remote); inbound flow
		// (responder is local) → listenerPID[local:listener_port].
		if epPID, ok := attr.outboundPIDFor(st.key.InitIP, st.key.RespIP); ok {
			addEndpointBytes(epPID, st, summary)
		}
		// Per-flow attribution (Phase 1): each unique
		// (localIP, remoteIP, remotePort) destination has its own
		// synthetic PID alongside the rollup. Populate FirstPacket
		// / LastPacket / BytesByEndpoint for it too — without this,
		// per-flow candidates have no time data and the timeline
		// cursor + the bar's per-flow time-range marking default
		// to the full pcap span (lighting up the whole bar even
		// when the destination only saw activity in a tiny slice).
		if flowPID, ok := attr.outboundFlowPIDFor(st.key.InitIP, st.key.RespIP, st.key.RespPort); ok {
			addEndpointBytes(flowPID, st, summary)
		}
		if epPID, ok := attr.listenerPID[attr.listenerKey(st.key.RespIP, st.key.RespPort)]; ok {
			// Reverse the FlowSummary's local/remote so the
			// listener candidate's flow list reads in its own
			// frame of reference (LocalIP = the listener, RemoteIP
			// = the connecting client).
			invSummary := FlowSummary{
				Proto:           "tcp",
				LocalIP:         st.key.RespIP,
				LocalPort:       st.key.RespPort,
				RemoteIP:        st.key.InitIP,
				RemotePort:      st.key.InitPort,
				BytesInitToResp: st.bytesRespToInit,
				BytesRespToInit: st.bytesInitToResp,
				FirstPacket:     unshift(st.firstPacket),
				LastPacket:      unshift(st.lastPacket),
			}
			addEndpointBytes(epPID, st, invSummary)
		}
	}
	result.ObservationByPID = make(map[int]time.Duration, len(firstSeen))
	for pid, first := range firstSeen {
		if last, ok := lastSeen[pid]; ok && last.After(first) {
			result.ObservationByPID[pid] = last.Sub(first)
		}
	}
	// DecisiveFlowsByPID — top-N flows per endpoint by total bytes.
	// Five is enough to surface the decisive C2/relay endpoints
	// without overwhelming the analyst.
	const topNDecisiveFlows = 5
	result.DecisiveFlowsByPID = make(map[int][]FlowSummary, len(flowsByEndpointPID))
	...
	for pid, list := range flowsByEndpointPID {
		// Compute beacon shape BEFORE the top-N truncation: the TS+DS
		// scores need the full flow history, not just the largest few.
		// Skip clusters with <4 flows (minimum for ≥3 intervals).
		if shape, ok := computeBeaconShape(list); ok {
			result.BeaconShapeByPID[pid] = shape
		}
		sort.Slice(list, func(i, j int) bool {
			ai := list[i].BytesInitToResp + list[i].BytesRespToInit
			aj := list[j].BytesInitToResp + list[j].BytesRespToInit
			return ai > aj
		})
		if len(list) > topNDecisiveFlows {
			list = list[:topNDecisiveFlows]
		}
		result.DecisiveFlowsByPID[pid] = list
	}
	...
	// Per-window byte tracking — chronologically-ordered series per
	// candidate that drives the activity sparkline + peak-window
	// detection. Computed in a single pass over the sorted flows
	// before the classify loop runs so it doesn't add any per-cycle
	// classification cost. Each flow contributes its bytes to every
	// window it overlaps (proportional to overlap duration); for the
	// chunky 1s windows we use, simple full-bytes-per-window-it-touches
	// is accurate enough for sparkline display and avoids fractional
	...
	// Use the per-flow per-window byte data captured during walkPcap
	// (st.bytesByWindow). Each entry is the ACTUAL bytes that
	// arrived in that window — not a uniform-spread estimate — so
	// quiet windows naturally show as zero and the bar's per-cell
	// coloring reflects real bursts and gaps. Window indices in
	// st.bytesByWindow are keyed off the original (pre-shift)
	// pcapStart, which matches the windows[] grid here.
	for _, st := range sortedFlows {
	...
	// Peak-window detection — the windowEnd timestamp of the largest
	// per-window bytes value per candidate. Used by the TUI to
	// suggest a Wireshark filter narrowed to the busiest interval.
	result.PeakWindowByPID = make(map[int]time.Time, len(attr.allPIDs))
	for pid, series := range result.PerWindowBytesByPID {
		var peakIdx int
		var peakVal uint64
		...
		if peakVal > 0 && peakIdx < len(windows) {
			result.PeakWindowByPID[pid] = unshift(windows[peakIdx])
		}
	}
	...
	for i, windowEnd := range windows {
		...
		// Per-window timeline summary — counts each candidate by its
		// computed state + role-family so the renderer can color the
		// strip without re-running CandidateState. Done before union
		// merging so we capture the state as it was THIS window
		// (post-Classify, pre-stabilization).
	...
	// Stabilization pass: replay the last window's snapshot a few more
	// times so the orchestrator's role-stability gates (stable-streak
	// counter, cooldowns, ML committed-role hold) converge before we
	// freeze the verdict. Without this, an edge-case role flip on the
	// final window of a long pcap can pin the candidate to a
	// transient verdict that doesn't reflect the bulk of the capture.
	...
	// Cross-candidate post-Classify promotion — parity with live capture's
	// shared.ScannerAdapter.Refresh() pipeline. detection.Classify() runs
	// per-candidate scoring + DemoteShapeOnlyControlRole, but the cross-
	// candidate steps (parent listener → pivot when child has
	// internal connections; relay child → pivot via PivotUntil
	// linger) are wired in only by the live-capture orchestrator, NOT
	// inside Classify. Without them, the pcap analyzer never promotes
	// the SSH-D session-handler sshd to pivot/tunneling — the
	// exact failure observed on test.pcap with cheerful_glove + sshd.
	//
	// Synthesize the processes map from the candidates' own Proc fields
	// so the 4-deep ancestor walk traverses pcap-synthetic parent chains
	// the same way it does in agent/server.go.
	...
	// Use PcapNow (anchored to pcap end time) so re-analysis of the
	// same pcap stamps PivotUntil / TunnelingSeen with identical
	// timestamps and produces deterministic findings.
	now := shared.PcapNow()
	scoring.AggregateChildTunnelEvidence(lastCands, processes, now)
	// Cross-candidate combo signal enrichment runs BEFORE ApplyPivotLinger
	// so the linger's pcap-mode HasPacketDecisiveSignal gate can see the
	// signals when deciding whether to stamp PivotUntil. Same for HTTP
	// fingerprint enrichment — its stamped signals (`http-c2-known-ua`,
	// `http-c2-uri-pattern`) are decisive and should arm the linger.
	...
	// Rare-signature scan: stamp tls-rare-signature / http-rare-signature
	// on clusters whose JA3/SNI/UA reaches only one external destination
	// from this host.
	enrichPcapWithRareSignatures(sortedFlows, attr, lastCands)
	scoring.ApplyPivotLinger(lastCands, processes, now)
	// Pcap-only: model.DecideRole overrides synthetic-PID candidates to
	// role="analyzing" because a brand-new process with no profile gets
	// held for evidence-gathering (decide.go:41 + the suspicious-role
	// hold at decide.go:146). That gate is correct for live capture
	// where an unknown process really should ramp up, but pcap analysis
	// is a one-shot — every synthetic PID is unknown and we never get
	// to gather evidence. Restore the pre-model rank.go suggestion so
	// the demote pass sees a clean signal-based role (matches tail-mode
	// ordering at line ~1210). Live PIDs are untouched.
	restoreSyntheticRoleFromAnalyzing(lastCands)
	// Pcap-only FP scrub: synthetic per-host outbound bundles for plain
	// admin SSH clients (high ephemeral → port 22) sometimes get pinned
	// to pivot via PivotUntil after a transient signal in some
	// intermediate window. Live mode never sees this because process
	// attribution distinguishes the SSH-client process from a relay
	// process; pcap mode lumps all of a host's internal-only outbound
	// into one synthetic PID with no way to differentiate. Demote
	// synthetic-PID candidates whose ONLY pivot evidence is the bare
	// pivot-non-loopback-internal signal — a real pivot has at least
	// one corroborating shape signal (multi-port fanout, SMB pivot,
	// reverse-tunnel direction, throughput symmetry, or child relay
	// context). Live PIDs are untouched.
	demoteUnderEvidencedSyntheticPivots(lastCands)
	// Re-run the pcap-mode role guard. AggregateChildTunnelEvidence +
	// ApplyPivotLinger above can re-promote candidates to pivot
	// AFTER detection.Classify already ran the guard internally (parent-
	// listener-with-child topology and PivotUntil linger both bypass the
	// per-candidate role assignment). The guard runs AFTER demote so it
	// has a clean signal-based role to evaluate, AND BEFORE host-C2
	// promotion so cheerful_glove's external CDN cluster gets rescued
	// from outbound → beacon via shared.ShouldRescuePcap before
	// host-C2 scans for active C2. This is the same order as tail mode
	// at line ~1241 — keeping the two pipelines in lock-step ensures
	// oneshot and continuous produce identical findings on the same
	// pcap input (operator request 2026-05-03).
	shared.ApplyPcapModeRoleGuard(lastCands)
	// Host-C2-active pivot promotion: when a host has a confirmed
	// beacon cluster, its internal-target clusters are the
	// implant's relay traffic — promote so the SOCKS-tunneled lateral
	// movement is surfaced alongside the C2 callback.
	promoteHostPivotsWhenC2Active(lastCands)
	// Statistical beacon-shape signals (TS+DS sub-scores) stamp on the
	// candidate when its flow distribution is uniform enough to be a
	// real beacon. Runs after rule-engine promotions so the new signals
	// supplement (not replace) existing beacon-* signals.
	stampBeaconShapeSignals(lastCands, result.BeaconShapeByPID)
	// Day-rhythm + strobe + prevalence signals (PR-B). Each is
	// independent of the TS/DS shape — strobe specifically catches
	// fast-pattern beacons that the interval-statistical signal scores
	// LOW (uniform ≠ slow).
	...
	// SMB lateral movement detection for internal agent-to-agent C2
	stampSMBLateralSignals(lastCands, sortedFlows, attr)
	// PR-C: open-conn cross-boundary signal — runs LAST among the
	// shape stampers so its decisive-evidence gate sees every other
	// signal that could arm it (TS/DS scores, strobe, long-conn,
	// rare-sig, etc.).
	stampOpenBoundarySignals(lastCands, result.OpenConnByPID)
	// Re-run the pcap-mode role guard a SECOND time after the shape
	// stampers. Path D's D2/D3/D4 conjunctions depend on signals
	// (`beacon-interval-statistical`, `beacon-payload-size-uniform`,
	// `beacon-day-rhythm`, `beacon-shape-high-confidence`) that get
	// stamped above — the first guard call at line ~977 ran BEFORE
	// these existed, so Path D had no ammunition. Operator-confirmed
	// 2026-05-04 on mythic_24hr.pcap: cluster had perfect shape
	// (TS=1.00, DS=1.00, 7610 flows, http-rare-signature, day-rhythm)
	// but stayed at outbound because the role guard ran too early in
	// the pipeline. The second call lets Path D fire on the populated
	// signal set. Safe because ApplyPcapModeRoleGuard's demote path
	// only fires when HasPacketDecisiveSignal goes false — adding
	// signals can only PROMOTE further, never demote.
	shared.ApplyPcapModeRoleGuard(lastCands)
	// Final FP scrub: kill LAN-discovery FPs (Chromecast / printer /
	// mDNS / SSDP / NetBIOS) that any earlier path may have promoted.
	// Runs AFTER all promotions but BEFORE operator labels so the
	// operator's malicious label still overrides this demote.
	demoteBenignLANDiscoveryClusters(lastCands)
	// Operator-label override (LAST in the chain): a `malicious`
	// label force-promotes; a `benign` label force-demotes. Runs
	// after every other pass so its verdict can't be reverted.
	applyPcapOperatorLabels(lastCands)
	// Re-merge promoted roles' new signals/reasons (e.g. child-tunnel-relay
	// added by the aggregation, "Pivot active" appended by linger) into
	// the union maps so the UI's pcap findings filter sees them.
	...
	// Refresh the debug API snapshot with the post-pass state.
	// detection.Classify's internal call to EmitDetectionOutputs captured
	// the candidates BEFORE restoreSyntheticRoleFromAnalyzing /
	// demoteUnderEvidencedSyntheticPivots / ApplyPcapModeRoleGuard ran,
	// so /candidates would otherwise show synthetic PIDs frozen at
	// role="analyzing" (the model's evidence-collecting hold) even though
	// the dashboard / TUI shows the correct post-pass role. Re-emitting
	// here closes that gap so external API consumers see the same state
	// the user sees in the TUI.
	//
	// Hold ClassifyMu.RLock around the call: UpdateDebugAPISnapshot →
	// CandidatesToSnapshots → candidateToSnapshot → CandidateStateUnsafe
	// reads ProcHistoryByPID / TunnelingSeen / ConnFirstSeen which the
	// live ScannerAdapter.Refresh writes under ClassifyMu.Lock when the
	// pcap analyzer is running side-by-side with live capture in the
	// TUI (the user's normal "Continuous" mode). Without the lock the
	// concurrent map access panics — observed live 2026-05-01.
	shared.ClassifyMu.RLock()
...
// TailCycle is how often a watch session re-runs detection.Classify
// against the accumulated flow state — once per second matches the
// orchestrator's natural per-snapshot cadence and keeps the UI lively
// without burning CPU. Reads happen continuously; only the
// classification + result-emission step is gated on this interval.
const TailCycle = 1 * time.Second

// IngestTail opens a still-being-written pcap file and follows it
// indefinitely, emitting an updated IngestResult onto resultCh once
// per TailCycle as new packets accumulate. Closes both channels on
// exit so the consumer can detect end-of-stream.
//
// Unlike Ingest, the underlying file is read through tailReader: when
// the writer pauses or hasn't appended new data yet, the reader
// blocks instead of returning EOF, so pcapgo never sees a short read.
// Cancellation via ctx is the only clean shutdown path.
//
// The orchestrator's history maps accumulate organically across
// cycles, so timing-based detectors (beacon-interval, syn-cycle,
// session-persistence) work the same as in live capture. No
// timestamp shifting is applied — packets in a still-being-written
// file are by definition timestamped near `time.Now()`.
//
// Synthetic-PID state is reset at start and cleaned up on exit, so
// repeated watch sessions on the same file are deterministic AND
// don't leak into live-capture state.
func IngestTail(ctx context.Context, path string, progressCh chan<- IngestProgress, resultCh chan<- IngestResult) {
	// Same DNS shutdown as the oneshot path — see IngestWithProgress
	// for full rationale. Tail mode is the user's "Continuous" mode
	// which is where the crash was first observed.
	shared.PcapModeActive.Store(true)
	defer shared.PcapModeActive.Store(false)
	defer func() {
		if progressCh != nil {
	...
	tail, file, err := newTailReader(ctx, path, 100*time.Millisecond, 0)
	if err != nil {
		emit(IngestResult{Path: path, Err: fmt.Errorf("open pcap: %w", err)})
		return
	}
	defer file.Close()

	reader, perr := pcapgo.NewReader(tail)
	var ngReader *pcapgo.NgReader
	if perr != nil {
		// Rewind and try pcapng. Note: since tail.Read blocks on EOF,
		// pcapgo header detection still works — it just sees the
		// header bytes that have been written so far.
		if _, err := file.Seek(0, 0); err != nil {
			emit(IngestResult{Path: path, Err: fmt.Errorf("rewind: %w", err)})
			return
		}
		var ngErr error
		ngReader, ngErr = pcapgo.NewNgReader(tail, pcapgo.DefaultNgReaderOptions)
		if ngErr != nil {
			emit(IngestResult{Path: path, Err: fmt.Errorf("not a pcap or pcapng (legacy: %v, ng: %v)", perr, ngErr)})
			return
		}
	}

	// Establish synthetic-PID universe lazily as new local IPs appear
	// in flows. Pre-allocate the cleanup-target slice so we always wipe
	// what we touched even if the user hits Esc before any packets
	// arrive.
	flows := make(map[flowKey]*flowState)
	dnsByHost := make(map[string]*dnsHostStats)
	// flowsMu guards the flows map. Tail mode's reader goroutine and
	// the main classify-ticker BOTH access it concurrently — the
	// reader appends new entries via processPacket while the ticker
	// iterates via sortFlowsForReplay. Go's map type does not support
	// concurrent read-during-write, and the runtime detects the race
	// and panics ("fatal error: concurrent map read and map write").
	// The previous "single-writer single-reader without mutex"
	// pattern (per the now-removed comment block) was unsafe by Go
	// memory-model rules — observed crash 2026-05-01 on host DEMO
	// during tail-mode startup with a 50+ MB live.pcap.
	var flowsMu sync.RWMutex
	pidByIP := make(map[string]int)
	var localIPs []string
	var allPIDs []int

	prevStartedAt := shared.ProxywatchStartedAt
	shared.ProxywatchStartedAt = time.Now().Add(-10 * shared.StartupGracePeriod)
	defer func() { shared.ProxywatchStartedAt = prevStartedAt }()
	defer func() { resetSyntheticPIDState(allPIDs) }()
	...
	// Reader runs in its own goroutine — pcapgo's ReadPacketData
	// blocks (via tailReader) and we need the main loop free to fire
	// classification cycles on a wallclock timer. The reader is the
	// sole writer to `flows`; the main loop snapshots it under the
	// classifyTicker. Single-writer single-reader pattern lets us
	// avoid mutex overhead on the hot packet path.
	...
	go func() {
		for {
			...
			lt := linkType
			if reader == nil {
				lt = ngReader.LinkType()
			}
			flowsMu.Lock()
			processPacket(data, lt, ci.Timestamp, pcapStart, packets, flows, dnsByHost)
			flowsMu.Unlock()
		}
	}()

	classifyTicker := time.NewTicker(TailCycle)
	defer classifyTicker.Stop()

	cycleCount := 0
	// timeline accumulates one TimelineWindow per classify cycle so
	// the UI's bar fills in left-to-right as the watch session runs.
	// Reused across cycles — buildTailResult takes it by value into
	// IngestResult.Timeline so the renderer always sees the full
	// history, not just the latest cycle.
	var timeline []TimelineWindow
	var lastAttr *pcapAttribution
	for {
		select {
		case <-ctx.Done():
			return
		case err := <-readerErr:
			// Reader exited (file gone, parse error, ctx cancel via
			// tailReader returning EOF). Emit a final result so the
			// UI can show the last frame, then exit. buildTailResult
			// iterates `flows` so we must hold the lock — the reader
			// goroutine has already exited by this point but the lock
			// is cheap and keeps the access pattern uniform.
			flowsMu.Lock()
			final := buildTailResult(path, packets, flows, pidByIP, localIPs, allPIDs,
				unionSignals, unionReasons, pcapStart, pcapEnd, nil, lastAttr, timeline)
			...
			emit(final)
			send(IngestProgress{Stage: "done", PacketsProcessed: packets, FlowsObserved: flowsCount})
			return
		case <-classifyTicker.C:
			cycleCount++
			// Lock flows for the duration of this tick. Reader
			// goroutine will block on its per-packet Lock while we're
			// here; tick durations are sub-second on typical hardware
			// so the OS receive buffer absorbs the gap. Without the
			// lock, refreshSyntheticPIDs / sortFlowsForReplay / the
			// flowState field reads further down race with the
			// reader's processPacket writes and crash the runtime.
			flowsMu.Lock()
			// Refresh synthetic-PID assignment for any new IPs seen.
			localIPs, pidByIP, allPIDs = refreshSyntheticPIDs(flows, pidByIP, allPIDs)
			if len(pidByIP) == 0 {
				// No flows yet — emit a heartbeat result so the UI
				// stays alive and the operator sees packets ticking.
				// Append an empty (quiet) timeline window so the bar
				// shows "we're alive but nothing here" instead of the
				// "Awaiting capture data..." placeholder.
				timeline = append(timeline, TimelineWindow{WindowEndAt: time.Now()})
				emit(buildTailResult(path, packets, flows, pidByIP, localIPs, allPIDs,
					unionSignals, unionReasons, pcapStart, pcapEnd, nil, lastAttr, timeline))
				flowsMu.Unlock()
				continue
			}
			sortedFlows := sortFlowsForReplay(flows)
			// Tail mode: rebuild per-endpoint attribution each cycle
			// because new listener ports / outbound IPs may have
			// appeared since the last sample. The attribution itself
			// is cheap (one pass over flows) compared to the actual
			// classification work that follows.
			tailAttr := buildPcapAttribution(sortedFlows, localIPs)
			lastAttr = tailAttr
			cycleNow := time.Now()
			snap := buildSnapshotForWindow(sortedFlows, tailAttr, localIPs, cycleNow, nil)
			cands := detection.Classify(snap, shared.ClassifyOptions{HostScope: "pcap-replay"}, nil)
			// Match the one-shot pipeline: run the cross-candidate
			// promotion (parent-listener → pivot, relay-child
			// pivot linger) before emitting. Without it, sshd's tunnel
			// shape never gets promoted in tail mode either.
			tailProcesses := make(map[int]*shared.ProcessInfo, len(cands))
			for i := range cands {
				if cands[i].Proc != nil {
					tailProcesses[cands[i].Proc.Pid] = cands[i].Proc
				}
			}
			scoring.AggregateChildTunnelEvidence(cands, tailProcesses, cycleNow)
			// Cross-candidate enrichment + HTTP fingerprint enrichment
			// run BEFORE ApplyPivotLinger so the new decisive signals
			// arm the linger's pcap-mode gate.
			enrichPcapWithCrossCandidatePivotSignals(cands)
			enrichPcapWithHTTPSignals(sortedFlows, tailAttr, cands)
			enrichPcapWithTLSSignals(sortedFlows, tailAttr, cands)
			enrichPcapWithSSHBannerSignals(sortedFlows, tailAttr, cands)
			enrichPcapWithDNSSignals(dnsByHost, cands)
			enrichPcapWithRareSignatures(sortedFlows, tailAttr, cands)
			scoring.ApplyPivotLinger(cands, tailProcesses, cycleNow)
			// Restore the pre-model signal-based role for synthetic
			// candidates the model held at "analyzing". Without this,
			// tail mode pins every cluster at "analyzing" forever
			// because the model's evidence-gathering hold never lifts
			// for a synthetic PID — the same hold the oneshot pipeline
			// resolves at line ~808. Run BEFORE
			// demoteUnderEvidencedSyntheticPivots so the demote pass
			// sees the proper role and can decide whether to keep it.
			restoreSyntheticRoleFromAnalyzing(cands)
			// Strict pcap-mode FP filter: demote beacon-* on synthetic
			// candidates that lack hard-distinguisher signals or packet-
			// decisive evidence. Mirrors the oneshot path (see line ~808)
			// — without it tail mode keeps cloud-SaaS / admin-IT outbound
			// flows pinned at beacon-* across many ticks because the
			// per-flow signals (beacon-shape, persistent-control, mixed-
			// protocol-internal) keep refiring on cumulative flow data.
			demoteUnderEvidencedSyntheticPivots(cands)
			// (Cross-candidate enrichment + HTTP fingerprint moved
			// upstream — they run before ApplyPivotLinger now.)
			// Re-apply the pcap-mode role guard after the post-passes.
			// AggregateChildTunnelEvidence + ApplyPivotLinger can promote
			// candidates to pivot via PivotUntil linger or parent-
			// listener-with-child topology AFTER Classify's internal guard
			// already settled the role. In tail mode the linger window is
			// 60s — without re-running the guard here, every per-flow
			// candidate that ever tripped pivot-non-loopback-internal
			// stays pinned at pivot for a minute regardless of
			// whether a packet-decisive signal is present.
			//
			// Order: role guard runs BEFORE host-C2 promotion because
			// the guard rescues cheerful_glove's external CDN cluster
			// from outbound → beacon via the
			// shared.ShouldRescuePcap signal-evidence path. host-C2
			// promotion needs that promotion to be visible (it scans
			// for c.Role == "beacon" to identify hosts with
			// active C2). If we ran host-C2 first, cheerful's cluster
			// would still be outbound and the host wouldn't be marked
			// C2-active — internal-target clusters would miss
			// promotion this cycle.
			shared.ApplyPcapModeRoleGuard(cands)
			// Host-C2-active pivot promotion: if this host has any
			// confirmed beacon cluster, its internal-target
			// clusters are the implant's relay traffic. Promote them
			// so the operator sees the SOCKS-tunneled lateral
			// movement, not just the C2 callback in isolation.
			promoteHostPivotsWhenC2Active(cands)
			// Statistical beacon-shape signals — tail mode recomputes
			// per-cycle from the current flow set since the IngestResult
			// isn't built until after the post-passes.
			tailShapes := computeBeaconShapesForFlows(flows, tailAttr)
			stampBeaconShapeSignals(cands, tailShapes)
			// PR-B: strobe + prevalence in tail mode. Day-rhythm is
			// skipped because hour-buckets aren't populated for tail
			// (it's a long-window pcap-replay signal, not a tail one).
			tailFlowsByPID := flowsByPIDForTail(flows, tailAttr)
			stampStrobeSignals(cands, tailFlowsByPID)
			tailFlowSlice := flowsToSlice(flows)
			stampPrevalenceSignals(cands, tailFlowSlice)
			stampOpenBoundarySignals(cands, countOpenConnsByPID(tailFlowSlice, tailAttr))
			// Final LAN-discovery FP scrub — see oneshot path comment.
			demoteBenignLANDiscoveryClusters(cands)
			// Operator-label override — final say. malicious = force
			// beacon-*; benign = force outbound. Runs LAST so no
			// downstream pass can revert the operator's verdict.
			applyPcapOperatorLabels(cands)
			for _, c := range cands {
				if c.Proc == nil {
					continue
				}
			...
			// Refresh debug API snapshot with the post-pass state — same
			// rationale as the oneshot path: detection.Classify's
			// internal EmitDetectionOutputs call ran with the model's
			// "analyzing" hold still in effect on synthetic PIDs.
			//
			// Hold ClassifyMu.RLock around the call (see oneshot path
			// for full rationale): CandidateStateUnsafe inside the
			// snapshot builder reads classifier-shared maps that the
			// live scanner writes under ClassifyMu.Lock — without the
			// read lock the tail-mode loop panics on concurrent map
			// access whenever the user has the live dashboard running
			// next to the pcap analyzer.
			shared.ClassifyMu.RLock()
			output.UpdateDebugAPISnapshot(output.NextDetectionOutputCycle(), "pcap-replay", cands)
			shared.ClassifyMu.RUnlock()
			// Append this cycle's classification summary so the
			// timeline bar paints another cell on next render.
			timeline = append(timeline, summarizeWindow(cycleNow, cands))
			emit(buildTailResult(path, packets, flows, pidByIP, localIPs, allPIDs,
				unionSignals, unionReasons, pcapStart, pcapEnd, cands, lastAttr, timeline))
			flowsCount := len(flows)
			flowsMu.Unlock()
			send(IngestProgress{
				Stage:            "watching",
				PacketsProcessed: packets,
				FlowsObserved:    flowsCount,
				WindowsReplayed:  cycleCount,
			})
		}
	}
}

// refreshSyntheticPIDs reconciles the pidByIP map with the latest set
// of local IPs inferred from the current flow state. Returns the
// (possibly extended) localIPs slice + pidByIP map + allPIDs slice
// that tracks every PID we've allocated for cleanup on exit.
func refreshSyntheticPIDs(flows map[flowKey]*flowState, pidByIP map[string]int, allPIDs []int) ([]string, map[string]int, []int) {
	localIPs := inferLocalIPs(flows)
	for _, ip := range localIPs {
		if _, ok := pidByIP[ip]; ok {
			continue
		}
		pid := SyntheticPIDBase + len(pidByIP)
		pidByIP[ip] = pid
		allPIDs = append(allPIDs, pid)
	}
	return localIPs, pidByIP, allPIDs
}

// buildTailResult assembles the per-cycle IngestResult emitted to the
// UI. Mirrors the per-PID stat aggregation used by the one-shot
// Ingest path so the packet-view columns, the FINDINGS table TIME /
// ID columns, and the timeline bar see identical-shape data.
//
// `attr` is the latest pcapAttribution for the cycle (nil when no
// flows have been seen yet — the heartbeat case). `timeline` is the
// running list of TimelineWindow summaries the cycle loop accumulates
// across cycles; the result reuses the same slice so the UI's bar
// fills in left-to-right as new windows are classified.
func buildTailResult(
	path string,
	packets uint64,
	flows map[flowKey]*flowState,
	pidByIP map[string]int,
	localIPs []string,
	allPIDs []int,
	unionSignals, unionReasons map[int][]string,
	pcapStart, pcapEnd time.Time,
	...
	res := IngestResult{
		...
		LocalIPs:         localIPs,
		SyntheticPIDs:    append([]int(nil), allPIDs...),
		Candidates:       cands,
		UnionSignals:     unionSignals,
		UnionReasons:     unionReasons,
		PcapStart:        pcapStart,
		PcapEnd:          pcapEnd,
		Timeline:         timeline,
		PacketsByPID:     make(map[int]uint64, len(pidByIP)),
		FlowsByPID:       make(map[int]int, len(pidByIP)),
		BytesByPID:       make(map[int]uint64, len(pidByIP)),
		BytesInByPID:     make(map[int]uint64, len(pidByIP)),
		BytesOutByPID:    make(map[int]uint64, len(pidByIP)),
		ObservationByPID: make(map[int]time.Duration, len(pidByIP)),
	}
	// Endpoint-PID-keyed maps drive the FINDINGS table (TIME, ID, BYTES,
	// LOCAL/REMOTE) and the timeline bar (PerWindowBytesByPID). Same
	// shape as the one-shot Ingest path so the renderer doesn't have
	// to special-case watch mode.
	res.FirstPacketByPID = make(map[int]time.Time)
	res.LastPacketByPID = make(map[int]time.Time)
	res.FirstFrameByPID = make(map[int]uint64)
	res.BytesByEndpointPID = make(map[int]uint64)
	res.PerWindowBytesByPID = make(map[int][]uint64)
	res.DecisiveFlowsByPID = make(map[int][]FlowSummary)

	firstSeen := make(map[int]time.Time, len(pidByIP))
	lastSeen := make(map[int]time.Time, len(pidByIP))
	bumpRange := func(pid int, first, last time.Time) {
		if existing, ok := firstSeen[pid]; !ok || first.Before(existing) {
			firstSeen[pid] = first
		}
		if existing, ok := lastSeen[pid]; !ok || last.After(existing) {
			lastSeen[pid] = last
		}
	}
	bumpEndpoint := func(pid int, st *flowState, summary FlowSummary) {
		if existing, ok := res.FirstPacketByPID[pid]; !ok || st.firstPacket.Before(existing) {
			res.FirstPacketByPID[pid] = st.firstPacket
		}
		if existing, ok := res.LastPacketByPID[pid]; !ok || st.lastPacket.After(existing) {
			res.LastPacketByPID[pid] = st.lastPacket
		}
		if st.firstFrameNum > 0 {
			if existing, ok := res.FirstFrameByPID[pid]; !ok || st.firstFrameNum < existing {
				res.FirstFrameByPID[pid] = st.firstFrameNum
			}
		}
		res.BytesByEndpointPID[pid] += st.bytesInitToResp + st.bytesRespToInit
		res.DecisiveFlowsByPID[pid] = append(res.DecisiveFlowsByPID[pid], summary)
	}
	for _, st := range flows {
		flowBytes := st.bytesInitToResp + st.bytesRespToInit
		flowPackets := uint64(st.packetsTotal)
		if pid, ok := pidByIP[st.key.InitIP]; ok {
			res.PacketsByPID[pid] += flowPackets
			res.FlowsByPID[pid]++
			res.BytesByPID[pid] += flowBytes
			res.BytesOutByPID[pid] += st.bytesInitToResp
			res.BytesInByPID[pid] += st.bytesRespToInit
			bumpRange(pid, st.firstPacket, st.lastPacket)
		}
		if pid, ok := pidByIP[st.key.RespIP]; ok && st.key.InitIP != st.key.RespIP {
			res.PacketsByPID[pid] += flowPackets
			res.FlowsByPID[pid]++
			res.BytesByPID[pid] += flowBytes
			res.BytesOutByPID[pid] += st.bytesRespToInit
			res.BytesInByPID[pid] += st.bytesInitToResp
			bumpRange(pid, st.firstPacket, st.lastPacket)
		}
		if attr == nil {
			continue
		...
		summary := FlowSummary{
			...
			BytesInitToResp: st.bytesInitToResp,
			BytesRespToInit: st.bytesRespToInit,
			FirstPacket:     st.firstPacket,
			LastPacket:      st.lastPacket,
		}
		if epPID, ok := attr.outboundPIDFor(st.key.InitIP, st.key.RespIP); ok {
			bumpEndpoint(epPID, st, summary)
		}
		if flowPID, ok := attr.outboundFlowPIDFor(st.key.InitIP, st.key.RespIP, st.key.RespPort); ok {
			bumpEndpoint(flowPID, st, summary)
		}
		if epPID, ok := attr.listenerPID[attr.listenerKey(st.key.RespIP, st.key.RespPort)]; ok {
			invSummary := FlowSummary{
				Proto:           "tcp",
			...
			bumpEndpoint(epPID, st, invSummary)
		}
	}
	for pid, first := range firstSeen {
		if last, ok := lastSeen[pid]; ok && last.After(first) {
			res.ObservationByPID[pid] = last.Sub(first)
		}
	}
	return res
}

// walkPcap reads every packet from the file, decodes IPv4/IPv6 + TCP, and
// builds per-flow state keyed by (initiator-side, responder-side) tuple.
// Non-TCP packets are skipped silently. Malformed link layers are
// reported as warnings but don't abort the walk.
...
// computeBeaconShapesForFlows builds a per-PID BeaconShape map from
// the live flow set. Tail-mode equivalent of the post-iteration
// computation in oneshot mode — gathers flow summaries grouped by
// outbound cluster PID (the granularity beacon shape is meaningful
// at) and feeds them through computeBeaconShape. Skips clusters with
// <4 flows (minimum for meaningful scoring).
func computeBeaconShapesForFlows(flows map[flowKey]*flowState, attr *pcapAttribution) map[int]BeaconShape {
	flowsByPID := flowsByPIDForTail(flows, attr)
	if len(flowsByPID) == 0 {
		return nil
	}
	out := make(map[int]BeaconShape, len(flowsByPID))
	for pid, list := range flowsByPID {
		if shape, ok := computeBeaconShape(list); ok {
			out[pid] = shape
		}
	}
...
// flowsByPIDForTail mirrors flowsByEndpointPID-construction in the
// oneshot path so the PR-B post-passes (strobe, day-rhythm) can run
// in tail mode against the current flow snapshot. Cluster-PID-keyed
// so a single flow that maps to both the rollup and the per-/16
// cluster gets entered under both — exactly what oneshot does.
func flowsByPIDForTail(flows map[flowKey]*flowState, attr *pcapAttribution) map[int][]FlowSummary {
	if attr == nil || len(flows) == 0 {
		return nil
	}
	out := make(map[int][]FlowSummary, 16)
	add := func(pid int, st *flowState) {
		out[pid] = append(out[pid], FlowSummary{
			LocalIP:         st.key.InitIP,
			LocalPort:       st.key.InitPort,
			RemoteIP:        st.key.RespIP,
			RemotePort:      st.key.RespPort,
...
// flowsToSlice flattens the tail flow map into a sortedFlows-style
// slice for the prevalence stamper (which iterates flows directly,
// not the per-PID grouping).
func flowsToSlice(flows map[flowKey]*flowState) []*flowState {
	if len(flows) == 0 {
		return nil
	}
...
func pcapProcName(ip string) string {
	return "pcap:" + ip
}
...
// IsPcapFile returns true if the path is a PCAP file.
func IsPcapFile(path string) bool {
	lower := strings.ToLower(path)
	return strings.HasSuffix(lower, ".pcap") || strings.HasSuffix(lower, ".pcapng")
}

// ParseLocalIP returns the parsed local-IP string, or "" if invalid. Used
// by the UI when validating a user-supplied capture-source override.
func ParseLocalIP(s string) string {
