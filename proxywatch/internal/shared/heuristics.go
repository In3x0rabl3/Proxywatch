...
// IsSSHClientProcess returns true for SSH client processes (not sshd servers).
// SSH clients connecting to internal infrastructure are normal admin activity.
func IsSSHClientProcess(p *ProcessInfo) bool {
	if p == nil {
		return false
	}
	...
	// Check vendor field for openssh-client
	if p.Company != "" && strings.Contains(strings.ToLower(p.Company), "openssh") {
		return true
	}
...
//
// SSH with ANY tunnel flags (-L, -R, -D, -w) is NOT suppressed because:
// - -L (local forward): can tunnel to internal services
// - -R (remote forward): reverse shell / C2 callback
// - -D (SOCKS proxy): enables full network pivoting
// - -w (tunnel device): creates network tunnel
//
// Only plain SSH sessions (no forwarding) are considered benign.
// Note: SSH baseline learning (in behavior/ssh_baseline.go) tracks patterns
// and adds signals like "ssh-first-time-destination" for new targets.
func IsSSHClientToInternal(c *Candidate) bool {
	if c == nil || c.Proc == nil {
		return false
	}
	...
	// Check for tunnel flags in command line - ANY tunnel flag means potential pivot
	cmdLower := strings.ToLower(c.Proc.CmdLine)
	// Note: SSH flags are case-sensitive (-L not -l), but we check lowercase
	// and also check for the actual flags
	hasTunnelFlags := strings.Contains(c.Proc.CmdLine, " -L") ||
		strings.Contains(c.Proc.CmdLine, " -R") ||
		strings.Contains(c.Proc.CmdLine, " -D") ||
		strings.Contains(c.Proc.CmdLine, " -w") ||
		strings.Contains(cmdLower, "localforward") ||
		strings.Contains(cmdLower, "remoteforward") ||
		strings.Contains(cmdLower, "dynamicforward")

	// If has ANY tunnel flags, don't suppress - could be malicious
	if hasTunnelFlags {
		return false
	}

	// Must be connecting to internal IP on port 22
	isInternalSSH := false
	if c.ControlChannel != nil {
		if IsInternalIP(c.ControlChannel.RemoteAddress) && c.ControlChannel.RemotePort == 22 {
			isInternalSSH = true
		}
	}
	if !isInternalSSH {
		for _, conn := range c.Conns {
			if IsInternalIP(conn.RemoteAddress) && conn.RemotePort == 22 {
				isInternalSSH = true
				break
			}
		}
...
func ApplyVendorUpdateSuppression(c *Candidate) bool {
	...
	// Only act when the current role is beacon-ish. Leave listener/outbound
