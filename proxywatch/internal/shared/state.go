...
type classifierMemoryDisk struct {
	...
	TunnelingSeen        map[int]time.Time           `json:"tunneling_seen,omitempty"`
	...
	DestFirstSeen        map[string]time.Time        `json:"dest_first_seen,omitempty"`
...
func LoadClassifierMemory(path string) error {
	...
	TunnelingSeen = cloneIntTimeMap(disk.TunnelingSeen, cutoff)
	...
	// DestFirstSeen — historical destination tracking. 90-day retention.
	// Older entries get pruned at load so first-seen scoring can't blame
	// a benign destination forever just because it was observed once
	// 6 months ago.
	destCutoff := time.Now().UTC().Add(-destFirstSeenRetention())
	DestFirstSeen = make(map[string]time.Time, len(disk.DestFirstSeen))
	for key, first := range disk.DestFirstSeen {
		if first.Before(destCutoff) {
			continue
		}
		DestFirstSeen[key] = first
	}
	...
	// Don't bump ModelGeneration on load — persisted processes already have
	// observations from previous sessions. Only baseline reset bumps generation.
...
func saveClassifierMemory(path string, now time.Time) error {
	...
	disk := classifierMemoryDisk{
		...
		TunnelingSeen:        cloneIntTimeMap(TunnelingSeen, time.Time{}),
	...
	// DestFirstSeen — copy the live map, dropping entries past the
	// 90-day retention so the disk file doesn't accumulate dead
	// destinations from systems we haven't talked to in months.
	destCutoff := now.Add(-destFirstSeenRetention())
	disk.DestFirstSeen = make(map[string]time.Time, len(DestFirstSeen))
	for key, first := range DestFirstSeen {
		if first.IsZero() || first.Before(destCutoff) {
			continue
		}
		disk.DestFirstSeen[key] = first
	}
...
// ClearClassifierState resets all classifier state maps. Called during
// RTB (Return to Baseline) so processes re-enter analyzing state with
// no inherited roles or status.
func ClearClassifierState() {
	ClassifyMu.Lock()
	defer ClassifyMu.Unlock()

	// Clear all role/status tracking maps
	PivotUntil = make(map[int]time.Time)
	TunnelingSeen = make(map[int]time.Time)
	TunnelActivitySeen = make(map[int]time.Time)
	PivotInternalSeen = make(map[int]time.Time)
	BeaconSeen = make(map[int]time.Time)
	...
	ConnFirstSeen = make(map[ConnKey]time.Time)
	ConnLastSeen = make(map[ConnKey]time.Time)
	ConnKeysByPID = make(map[int][]ConnKey)

	// Clear behavioral tracking
	RecentClientSeen = make(map[int]time.Time)
	RecentOutboundSeen = make(map[int]time.Time)
	RecentInternalScanSeen = make(map[int]time.Time)
	ShortLivedBurstLast = make(map[int]time.Time)
...
// destFirstSeenRetention controls how long destination first-seen
// entries stick in DestFirstSeen. 90 days is long enough to recognise
// vendor traffic that's been running for months, short enough that a
// destination contacted once 6 months ago doesn't suppress scoring
// forever.
func destFirstSeenRetention() time.Duration {
