...
// LookupCachedASNOrgsForPrefix returns cached ASN orgs for any IP in
// the cache whose first two octets match the given prefix. Accepts
// "104.21", "104.21.0.0", or "104.21.0.0/16" forms. Used by the pcap
// destinations panel where cluster names carry a /16 prefix rather
// than an exact IP — pcap ingest seeds the cache via QueueASNLookupsForIPs
// before the panel renders, so a representative match almost always
// exists.
func LookupCachedASNOrgsForPrefix(prefix string) []string {
	...
	twoOct := parts[0] + "." + parts[1] + "."
	asnCacheMu.RLock()
	defer asnCacheMu.RUnlock()
	for ip, rec := range asnCache {
		if rec.err != "" || len(rec.orgs) == 0 {
			continue
		}
		if !strings.HasPrefix(ip, twoOct) {
			continue
		}
		if time.Since(rec.updatedAt) > asnCacheTTL {
			continue
...
// QueueASNLookupsForIPs schedules async ASN resolution for the given
// raw IPs. Called by pcap ingest as it builds attribution so the cache
// is populated by the time the destinations panel reads it. Skips
// internal / loopback / wildcard IPs and IPs already cached.
func QueueASNLookupsForIPs(ips []string) {
	for _, ip := range ips {
		ip = strings.TrimSpace(ip)
		if ip == "" || IsWildcardIP(ip) || IsLoopbackIP(ip) || IsInternalIP(ip) {
			continue
		}
		getOrQueueASNRecord(ip)
	}
}
