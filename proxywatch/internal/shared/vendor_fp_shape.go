...
// whose identity is well-attested but behavior is classically beacon-
// channel-shaped.
...
func ClassifyVendorFPBlockers(c *Candidate) (hard []string, soft []string) {
	...
	if c.ControlChannel != nil && c.ControlDurationSeconds >= 60 {
		soft = append(soft, "state:sustained-session")
	}

	// ML high-confidence beacon-role = soft blocker. The model may be
	...
	if c.MLActive && c.MLConfidence >= 0.80 && IsControlRole(c.Role) {
		soft = append(soft, "state:ml-confirmed-beacon-role")
	}
...
func EvaluateVendorFPShape(c *Candidate, threshold int) FPShape {
	...
	// escapes the old "all beacon is decisive" trap.
	if len(soft) > 0 {
		if vendorCount >= SoftBlockerOverrideMin {
			...
			// consumable by log collectors; otherwise they collect in the
...
// IsTrustedSystemVendorIPCContext returns true when the candidate is a
// signature-trusted vendor binary in a system install path with no
// external network traffic and no implant-decisive signals. Used by
// child-tunnel evidence aggregation to skip the listener→pivot
// promotion for legitimate Windows IPC (services.exe → svchost children
// doing RPC, lsass.exe → SAM/LSA workers, etc.).
//
// This is a tighter gate than ApplyVendorIPCRescue: it requires zero
// external connections (a true system service has no business talking to
// the internet directly), so a compromised system binary that beacons out
// will fail this check and still get correctly promoted. An attacker
// abusing a system service for purely-internal pivoting would need to
// also avoid every implant-decisive signal — the same bar the rescue
// uses, but applied earlier in the pipeline before the FP role
// promotion actually fires.
func IsTrustedSystemVendorIPCContext(c *Candidate) bool {
	if c == nil || c.Proc == nil {
		return false
	}
...
// rich-local-ipc-shape rescue demotes a beacon-* role. Distinct from
...
// ApplyVendorIPCRescue demotes a shape-only beacon-* role to outbound /
...
func ApplyVendorIPCRescue(c *Candidate) bool {
	...
	// Demote. Reuse the existing role-aware target: pivot with
