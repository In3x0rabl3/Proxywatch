...
func fillTimes(h windows.Handle, pi *shared.ProcessInfo) {
	...
	if r, _, _ := platform.ProcGetProcessTimes.Call(
		uintptr(h),
...
func fillMemory(h windows.Handle, pi *shared.ProcessInfo) {
	var pmc platform.ProcessMemoryCounters
	pmc.Cb = uint32(unsafe.Sizeof(pmc))

	if r, _, _ := platform.ProcGetProcessMemoryInfo.Call(
		uintptr(h),
...
func fillIOCounters(h windows.Handle, pi *shared.ProcessInfo) {
	...
	if r, _, _ := platform.ProcGetProcessIoCounters.Call(
		uintptr(h),
...
func fillSession(pi *shared.ProcessInfo) {
	var sid uint32
	if r, _, _ := platform.ProcProcessIdToSessionId.Call(
...
func queryCompanyName(path string) (string, error) {
	...
	var handle uint32
	size, _, _ := platform.ProcGetFileVersionInfoSize.Call(
	...
	r1, _, _ := platform.ProcGetFileVersionInfo.Call(
		uintptr(unsafe.Pointer(pathPtr)),
		0,
		uintptr(size),
...
func queryVersionTranslation(buf []byte) (uint16, uint16, error) {
	...
	var valueLen uint32
	r0, _, _ := platform.ProcVerQueryValue.Call(
		uintptr(unsafe.Pointer(&buf[0])),
